...
test('shows token usage in the daily and weekly dashboard charts', async ({ page }) => {
  await page.route('**/api/source-modules/changes', (route) => route.fulfill({ json: [] }));
  await page.route('**/api/codex/requests/metrics**', (route) => {
    const window = { startsAt: '2026-09-14T05:00:00Z', requestCount: 1, interactionCount: 2, durationMs: 3_000, totalTokens: 12_345, weeklyQuotaConsumedPercentagePoints: 7.5 };
  ...
  await expect(page.getByRole('heading', { name: 'Cota semanal consumida' })).toHaveCount(2);
  await expect(page.getByLabel('14/09/26: 7,5 p.p.')).toHaveCount(2);
  await expect(page.getByLabel('Meta diária: 14 p.p.')).toHaveCount(1);
