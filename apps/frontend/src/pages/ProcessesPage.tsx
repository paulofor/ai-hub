...
interface ProcessRecord { id: number; number: string; text: string; updatedAt: string }
...
export default function ProcessesPage() {
  ...
  useEffect(() => { void load(); }, []);
  const reset = () => { setNumber(''); setProcessText(''); setEditing(null); };
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setSaving(true); setError(null);
    try {
      const payload = { number: number.trim(), text: processText.trim() };
      if (editing) await client.put(`/processes/${editing.id}`, payload); else await client.post('/processes', payload);
      reset(); await load();
    } catch (e) { setError((e as Error).message); } finally { setSaving(false); }
  };
  const edit = (item: ProcessRecord) => { setEditing(item); setNumber(item.number); setProcessText(item.text); };
  const remove = async (id: number) => { await client.delete(`/processes/${id}`); if (editing?.id === id) reset(); await load(); };
  return <div className="space-y-6">
    <div><h2 className="text-2xl font-semibold">Processos</h2><p className="text-sm text-slate-500">Cadastre os processos que podem ser associados às solicitações.</p></div>
    <form onSubmit={submit} className="space-y-4 rounded-xl border bg-white/70 p-5 dark:border-slate-800 dark:bg-slate-900/60">
      <div><label htmlFor="process-number" className="text-sm font-medium">Número</label><input id="process-number" required maxLength={80} value={number} onChange={e => setNumber(e.target.value)} className="mt-1 w-full rounded-md border px-3 py-2 dark:bg-slate-950" placeholder="Ex.: 12345/2026" /></div>
    ...
    <div className="overflow-x-auto rounded-xl border bg-white/70 dark:border-slate-800 dark:bg-slate-900/60"><table className="w-full text-left text-sm"><thead><tr className="border-b"><th className="px-4 py-3">Número</th><th className="px-4 py-3">Texto</th><th className="px-4 py-3">Ações</th></tr></thead><tbody>{items.map(item => <tr key={item.id} className="border-b last:border-0"><td className="px-4 py-3 font-medium">{item.number}</td><td className="px-4 py-3">{item.text}</td><td className="flex gap-3 px-4 py-3"><button onClick={() => edit(item)} className="text-emerald-700 hover:underline">Editar</button><ConfirmButton label="Excluir" confirmLabel="Confirmar exclusão" onConfirm={() => remove(item.id)} /></td></tr>)}{items.length === 0 && <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-500">Nenhum processo cadastrado.</td></tr>}</tbody></table></div>
  </div>;
}
