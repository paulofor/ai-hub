...
type Job = { jobId?: string; status?: string; profile?: string; model?: string; startedAt?: string; updatedAt?: string; runningForMs?: number; idleForMs?: number; interactionCount?: number; promptTokens?: number; cachedPromptTokens?: number; completionTokens?: number; totalTokens?: number; timeoutCount?: number; lastLog?: string; warnings?: string[] };
type Health = {
  generatedAt?: string;
  host?: CommandResult;
...
const formatNumber = (value?: number) => value === undefined ? '—' : new Intl.NumberFormat('pt-BR').format(value);
...
export default function SystemHealthPage() {
  ...
  return <div className="space-y-6">
    ...
    {health && <>
      ...
      <section className="rounded-xl border bg-white p-5 dark:bg-slate-950"><h3 className="font-bold">Jobs ativos</h3><p className="mt-1 text-sm text-slate-500">Tempo de execução, inatividade, tokens e última atividade informada pelo sandbox.</p>{activeJobs.length === 0 ? <p className="mt-2 text-sm text-slate-500">Nenhum job ativo.</p> : activeJobs.map((job) => { const stalled = job.warnings?.includes('NO_PROGRESS'); return <div key={job.jobId} className={`mt-3 rounded-lg border p-3 ${stalled ? 'border-rose-300 bg-rose-50 dark:border-rose-900 dark:bg-rose-950/20' : ''}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="font-mono text-sm">{job.jobId}</p>{stalled && <span className="rounded-full bg-rose-600 px-2 py-0.5 text-xs font-semibold text-white">Sem progresso</span>}{job.warnings?.includes('LONG_RUNNING') && <span className="rounded-full bg-amber-500 px-2 py-0.5 text-xs font-semibold text-white">Execução longa</span>}</div><p className="text-xs text-slate-500">{job.profile} · {job.model ?? 'modelo não informado'} · {job.status}</p></div><div className="flex gap-2"><button onClick={() => run('cancel-job', job.jobId, false)} disabled={!!action} className="rounded border px-3 py-1.5 text-sm">Cancelar</button><button onClick={() => run('force-cancel-job', job.jobId)} disabled={!!action} className="rounded bg-rose-600 px-3 py-1.5 text-sm text-white">Forçar encerramento</button></div></div><dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><dt className="text-xs uppercase text-slate-500">Em execução</dt><dd className="font-semibold">{formatDuration(job.runningForMs)}</dd></div><div><dt className="text-xs uppercase text-slate-500">Sem atividade</dt><dd className={stalled ? 'font-semibold text-rose-700' : 'font-semibold'}>{formatDuration(job.idleForMs)}</dd></div><div><dt className="text-xs uppercase text-slate-500">Tokens</dt><dd className="font-semibold">{formatNumber(job.totalTokens)}</dd></div><div><dt className="text-xs uppercase text-slate-500">Interações / timeouts</dt><dd className="font-semibold">{formatNumber(job.interactionCount)} / {formatNumber(job.timeoutCount)}</dd></div></dl><div className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-3"><p>Entrada: <strong>{formatNumber(job.promptTokens)}</strong></p><p>Cache: <strong>{formatNumber(job.cachedPromptTokens)}</strong></p><p>Saída: <strong>{formatNumber(job.completionTokens)}</strong></p></div>{job.lastLog && <p className="mt-3 break-words rounded bg-slate-100 p-2 font-mono text-xs text-slate-700 dark:bg-slate-900 dark:text-slate-300"><strong>Última atividade:</strong> {job.lastLog}</p>}</div>; })}</section>
