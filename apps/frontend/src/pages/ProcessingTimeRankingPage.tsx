...
export default function ProcessingTimeRankingPage() {
  const [items, setItems] = useState<ProcessingTimeRankingItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    client.get<ProcessingTimeRankingItem[]>('/codex/requests/processing-time-ranking')
  ...
  return (
    <section className="space-y-6">
      <header className="overflow-hidden rounded-2xl bg-gradient-to-br from-slate-950 via-sky-950 to-sky-700 p-6 text-white shadow-xl sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.24em] text-sky-200">Desempenho do Codex</p>
        <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
          <div><h2 className="text-3xl font-bold tracking-tight">Ranking por tempo de processamento</h2><p className="mt-2 max-w-2xl text-sm text-sky-50/80">As 20 solicitações com maior duração, ordenadas da mais demorada para a mais rápida.</p></div>
          <div className="rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-right backdrop-blur"><span className="block text-2xl font-bold">Top 20</span><span className="text-xs text-sky-100">atualizado ao abrir</span></div>
        </div>
      </header>
