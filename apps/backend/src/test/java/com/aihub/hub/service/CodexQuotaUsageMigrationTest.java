...
class CodexQuotaUsageMigrationTest {
    @Test
    void h2AndPostgresqlMigrationPreserveExistingRowsAndCanRunTwice() throws Exception {
        for (String dialect : new String[] { "h2", "postgresql" }) {
            try (var connection = DriverManager.getConnection("jdbc:h2:mem:quota-migration-" + dialect);
                ...
                String sql = Files.readString(Path.of("src/main/resources/db/migration", dialect,
                    "V52__add_quota_usage_to_codex_requests.sql"));
                statement.execute(sql);
                statement.execute(sql);
                try (var row = statement.executeQuery("SELECT response_text, quota_usage FROM codex_requests")) {
                    assertThat(row.next()).isTrue();
                    assertThat(row.getString(1)).isEqualTo("Resposta antiga");
                    assertThat(row.getString(2)).isNull();
                }
                String summary = "Resumo público.\n".repeat(5000);
                try (var update = connection.prepareStatement("UPDATE codex_requests SET quota_usage=? WHERE id=1")) {
                    update.setString(1, summary);
                    assertThat(update.executeUpdate()).isEqualTo(1);
                }
                try (var row = statement.executeQuery("SELECT quota_usage FROM codex_requests")) {
                    assertThat(row.next()).isTrue();
