...
export class SandboxJobProcessor implements JobProcessor {
  ...
  private buildCodexAppServerInput(job: SandboxJob): Array<Record<string, string>> {
    ...
    const marketingStructuredResponseInstruction = 'Na resposta final do modo MKT, responda somente com JSON válido no formato {"titulo":"<título muito curto, uma frase simples>","comentario":"<resposta principal em Markdown>","orientacaoProximaAcao":"<orientação objetiva para a próxima ação baseada na alternativa escolhida, ou string vazia se não houver orientação aplicável>","sugestaoMelhoriaAmbiente":"<sugestão de recurso ou ferramenta que teria permitido fazer um trabalho melhor durante a solicitação, ou string vazia se o ambiente já foi suficiente>"}. Use o campo comentario para todo o conteúdo normal da resposta. Use orientacaoProximaAcao apenas quando existir uma decisão entre alternativas ou uma próxima ação clara para orientar o usuário. Use sugestaoMelhoriaAmbiente apenas para melhoria do ambiente de execução.';
