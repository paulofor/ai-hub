...
export class SandboxJobProcessor implements JobProcessor {
  ...
  private buildGithubDeliveryInstruction(job: SandboxJob): string {
    return this.isChatgptCodexSandbox(job) ? '' : `${GITHUB_DELIVERY_INSTRUCTION} ${PRODUCTION_PUBLICATION_INSTRUCTION}`;
  }
...
${job.taskDescription}${this.buildAttachmentContext(job)}`
      : this.isChatgptCodex(job)
        ? `Modo Codex ChatGPT ativo: ${bestAnswerInstruction} ${localDevelopmentInstruction} ${codexChatgptOperationalInstruction} ${awsCliInstruction} ${externalApiKeysInstruction} ${dockerCliInstruction} ${liquibaseMysql57RunnerInstruction} ${sshClientInstruction} ${mediaToolsInstruction} ${browserTestingInstruction}
