...
- `POST /jobs`: cria um job informando `jobId`, `repoUrl` ou `repoSlug`, `branch`, `taskDescription` e (opcionalmente) `testCommand`/`commit`. O serviço clona o repositório em um diretório temporário, expõe as tools `run_shell`, `read_file`, `write_file` e `http_get` ao modelo e inicia o loop de tool-calling. A tool `http_get` permite consultas HTTP públicas, bloqueando hosts locais/privados e truncando respostas grandes.
  ...
  A publicação conserva o HEAD validado, inclusive commits criados pelo modelo em outra branch local. Uma branch remota existente precisa ser ancestral desse HEAD; avanço concorrente, falha de consulta ou push interrompem a publicação e marcam o job como `FAILED`, sem resetar o código para o remoto. O workspace e o patch ficam disponíveis para recuperação. Jobs com alterações sem credencial de publicação também conservam o workspace; a retenção depende da persistência de `SANDBOX_WORKDIR`, e sua remoção operacional deve ocorrer somente após recuperar o trabalho. A limpeza automática normal continua para jobs concluídos sem alterações ou com branch publicada e para o perfil sem repositório.
...
- Quando o modelo já integrou o trabalho na branch base, a finalização verifica que não há alterações locais pendentes e que o HEAD pertence ao histórico remoto atualizado ou tem exatamente a mesma árvore de código (squash/rebase); então conclui sem push/PR redundante, preservando o patch histórico. Uma checagem negativa em clone raso exige completar o histórico antes de classificar divergência. Falha de consulta ou histórico ainda incompleto mantém o código recuperável e o status `FAILED`.
- Se ocorrer erro após a resposta final, o backend preserva a resposta junto de um aviso explícito de falha no encerramento, mantendo `FAILED`. Respostas JSON MKT recebem o aviso dentro de `comentario`, para que a interface mostre ambos. Consulte a [matriz de encerramento](../../docs/homologacao-encerramento-solicitacoes.md).
...
| `SANDBOX_SSH_ALLOWED_DESTINATIONS` | Lista separada por vírgulas de destinos `usuario@host` carregados como restrições no agente e aceitos pelo helper `sandbox-ssh`. Cada host também precisa estar fixado em `/etc/sandbox-ssh/known_hosts`. | seis VPS operacionais versionadas no Compose |
...
O runner também inclui o `shellcheck` para análise estática de scripts shell. Ao alterar scripts, execute `bash -n` e depois `shellcheck` nos arquivos relevantes; a validação versionada `bash .github/scripts/validate-shell-scripts.sh` aplica as duas verificações aos scripts shell de primeira parte, excluindo código de terceiros/vendor.
