...
| `CODEX_APP_SERVER_TURN_NO_ACTIVITY_TIMEOUT_MS` | Tempo máximo sem nenhum evento/notificação útil do Codex App Server durante `turn/start`; tolera operações longas e ainda evita que execuções fiquem indefinidamente em `Em execução` quando o app-server deixa de responder. | `7200000` |
