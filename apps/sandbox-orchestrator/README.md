...
Os tokens Luma, Kling, HeyGen e Radar Meta devem ficar fora do repositório nos arquivos `/root/infra/luma-token/luma_api_key`, `/root/infra/kling-token/kling_api_key`, `/root/infra/heygen-token/heygen_api_key` e `/root/infra/radarmeta-token/radar_meta_token`. Quando os arquivos existem, o `sandbox-orchestrator` exporta `LUMA_API_KEY`, `KLING_API_KEY`, `HEYGEN_API_KEY` e `RADAR_META_TOKEN` para os comandos do modelo e para o Codex App Server; nunca registre os valores dessas variáveis em logs, respostas ou arquivos.
