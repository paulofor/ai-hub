...
test('infere testCommand com maven quando há alterações em módulo Java', async () => {
  const tempRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'sandbox-testcmd-infer-'));
  const processor = new SandboxJobProcessor(undefined, 'gpt-5-codex');

  try {
    const rootPom = path.join(tempRepo, 'pom.xml');
    await fs.writeFile(rootPom, '<project />');
    await fs.mkdir(path.join(tempRepo, 'src', 'main', 'java'), { recursive: true });
    await fs.writeFile(path.join(tempRepo, 'src', 'main', 'java', 'Root.java'), 'class Root {}');

    const rootJob: SandboxJob = {
      jobId: 'job-infer-root',
      repoUrl: tempRepo,
      branch: 'main',
      taskDescription: 'root module',
      status: 'PENDING',
      logs: [],
      interactions: [],
      interactionSequence: 0,
      ...
      updatedAt: new Date().toISOString(),
      timeoutCount: 0,
      changedFiles: ['src/main/java/Root.java'],
    } as SandboxJob;

    const rootCommand = await (processor as any).inferTestCommand(rootJob, tempRepo);
    assert.equal(rootCommand, 'mvn test');

    await fs.mkdir(path.join(tempRepo, 'service', 'src', 'main', 'java'), { recursive: true });
    await fs.writeFile(path.join(tempRepo, 'service', 'pom.xml'), '<project />');
    await fs.writeFile(path.join(tempRepo, 'service', 'src', 'main', 'java', 'Service.java'), 'class Service {}');

    const moduleJob: SandboxJob = {
      jobId: 'job-infer-module',
      repoUrl: tempRepo,
      branch: 'main',
      taskDescription: 'module change',
      status: 'PENDING',
      logs: [],
      interactions: [],
      interactionSequence: 0,
      ...
      updatedAt: new Date().toISOString(),
      timeoutCount: 0,
      changedFiles: ['service/src/main/java/Service.java'],
    } as SandboxJob;

    const moduleCommand = await (processor as any).inferTestCommand(moduleJob, tempRepo);
    assert.equal(moduleCommand, 'mvn -pl service -am test');

    await fs.mkdir(path.join(tempRepo, 'lib', 'src', 'main', 'java'), { recursive: true });
    await fs.writeFile(path.join(tempRepo, 'lib', 'pom.xml'), '<project />');
    await fs.writeFile(path.join(tempRepo, 'lib', 'src', 'main', 'java', 'Lib.java'), 'class Lib {}');

    const multiModuleJob: SandboxJob = {
      jobId: 'job-infer-multi',
      repoUrl: tempRepo,
      branch: 'main',
      taskDescription: 'multi module change',
      status: 'PENDING',
      logs: [],
      interactions: [],
      interactionSequence: 0,
    ...
    const multiCommand = await (processor as any).inferTestCommand(multiModuleJob, tempRepo);
    assert.equal(multiCommand, 'mvn -pl lib,service -am test');
  } finally {
    await fs.rm(tempRepo, { recursive: true, force: true });
  }
});
