...
test('preserves function_call pair when context GC keeps function_call_output', async () => {
  ...
  execSync('git config user.email "ci@example.com"', { cwd: tempRepo });
  execSync('git config user.name "CI Bot"', { cwd: tempRepo });
  await fs.writeFile(path.join(tempRepo, 'README.md'), 'context-gc');
  execSync('git add README.md', { cwd: tempRepo });
  execSync('git commit -m "init"', { cwd: tempRepo });
  execSync('git branch -M main', { cwd: tempRepo });
  ...
  try {
    ...
    const job: SandboxJob = {
      ...
      repoUrl: tempRepo,
      branch: 'main',
      taskDescription: 'read file with forced context gc',
      status: 'PENDING',
      logs: [],
      interactions: [],
      interactionSequence: 0,
    ...
    const secondCall = fakeOpenAI.calls[1];
    const functionCall = secondCall.input.find((msg: any) => msg.type === 'function_call' && msg.call_id === 'call-gc-1');
    const functionOutput = secondCall.input.find((msg: any) => msg.type === 'function_call_output' && msg.call_id === 'call-gc-1');
    assert.ok(functionCall, 'function_call pareado deve permanecer após GC de contexto');
    assert.ok(functionOutput, 'function_call_output deve permanecer após GC de contexto');
  } finally {
...
test('drops function_call when required reasoning item is missing from compacted history', async () => {
  const originalPromptLimit = process.env.CONTEXT_PROMPT_ESTIMATE_LIMIT;
  const originalRecentWindow = process.env.CONTEXT_RECENT_MESSAGE_WINDOW;
  const originalWorkingSetWindow = process.env.CONTEXT_WORKING_SET_WINDOW;
  process.env.CONTEXT_PROMPT_ESTIMATE_LIMIT = '1';
  ...
  const tempRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'sandbox-context-gc-orphan-reasoning-'));
  execSync('git init', { cwd: tempRepo });
  execSync('git config user.email "ci@example.com"', { cwd: tempRepo });
  execSync('git config user.name "CI Bot"', { cwd: tempRepo });
  await fs.writeFile(path.join(tempRepo, 'README.md'), 'context-gc-orphan-reasoning');
  execSync('git add README.md', { cwd: tempRepo });
  execSync('git commit -m "init"', { cwd: tempRepo });
  execSync('git branch -M main', { cwd: tempRepo });
        ...
        if (fakeOpenAI.calls.length === 1) {
          return {
            output: [
              {
                type: 'function_call',
                call_id: 'call-orphan-reasoning-1',
                name: 'read_file',
                reasoning: 'rs_missing_later',
                arguments: JSON.stringify({ path: 'README.md' }),
              },
              {
                type: 'message',
                id: 'msg-orphan-reasoning-1',
                role: 'assistant',
                status: 'completed',
                content: [{ type: 'output_text', text: 'x'.repeat(200), annotations: [] }],
              },
  ...
  try {
    const processor = new SandboxJobProcessor(undefined, 'gpt-5-codex', fakeOpenAI);
    const job: SandboxJob = {
      jobId: 'job-context-gc-orphan-reasoning',
      repoUrl: tempRepo,
      branch: 'main',
      taskDescription: 'read file with compacted orphan reasoning dependency',
      status: 'PENDING',
      logs: [],
      interactions: [],
      interactionSequence: 0,
    ...
    const secondCall = fakeOpenAI.calls[1];
    const functionCall = secondCall.input.find((msg: any) => msg.type === 'function_call' && msg.call_id === 'call-orphan-reasoning-1');
    const functionOutput = secondCall.input.find((msg: any) => msg.type === 'function_call_output' && msg.call_id === 'call-orphan-reasoning-1');
    assert.equal(functionCall, undefined, 'function_call órfão deve ser removido antes do próximo turno');
    assert.equal(functionOutput, undefined, 'function_call_output órfão também deve ser removido');
  } finally {
    if (originalPromptLimit === undefined) {
      delete process.env.CONTEXT_PROMPT_ESTIMATE_LIMIT;
    } else {
...
test('detects reasoning refs embedded in raw function_call payload and removes orphan pair', async () => {
  const tempRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'sandbox-context-gc-hidden-reasoning-'));
  execSync('git init', { cwd: tempRepo });
  execSync('git config user.email "ci@example.com"', { cwd: tempRepo });
  execSync('git config user.name "CI Bot"', { cwd: tempRepo });
  await fs.writeFile(path.join(tempRepo, 'README.md'), 'hidden-reasoning');
  execSync('git add README.md', { cwd: tempRepo });
  execSync('git commit -m "init"', { cwd: tempRepo });
  execSync('git branch -M main', { cwd: tempRepo });
        ...
        if (fakeOpenAI.calls.length === 1) {
          return {
            output: [
              {
                type: 'function_call',
                call_id: 'call-hidden-reasoning-1',
                name: 'read_file',
                extra: { refs: [{ kind: 'required', ref: 'rs_hidden_required_1' }] },
                arguments: JSON.stringify({ path: 'README.md' }),
              },
            ],
          };
  ...
  try {
    const processor = new SandboxJobProcessor(undefined, 'gpt-5-codex', fakeOpenAI);
    const job: SandboxJob = {
      jobId: 'job-context-gc-hidden-reasoning',
      repoUrl: tempRepo,
      branch: 'main',
      taskDescription: 'read file with hidden orphan reasoning dependency',
      status: 'PENDING',
      logs: [],
      interactions: [],
      interactionSequence: 0,
    ...
    const secondCall = fakeOpenAI.calls[1];
    const functionCall = secondCall.input.find((msg: any) => msg.type === 'function_call' && msg.call_id === 'call-hidden-reasoning-1');
    const functionOutput = secondCall.input.find((msg: any) => msg.type === 'function_call_output' && msg.call_id === 'call-hidden-reasoning-1');
    assert.equal(functionCall, undefined, 'function_call com reasoning implícito órfão deve ser removido');
    assert.equal(functionOutput, undefined, 'function_call_output pareado também deve ser removido');
  } finally {
    await fs.rm(tempRepo, { recursive: true, force: true });
  }
});

test('retries request when provider reports missing reasoning dependency for function_call id', async () => {
  const tempRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'sandbox-provider-missing-reasoning-'));
  execSync('git init', { cwd: tempRepo });
  execSync('git config user.email "ci@example.com"', { cwd: tempRepo });
  execSync('git config user.name "CI Bot"', { cwd: tempRepo });
  await fs.writeFile(path.join(tempRepo, 'README.md'), 'provider-missing-reasoning');
  execSync('git add README.md', { cwd: tempRepo });
  execSync('git commit -m "init"', { cwd: tempRepo });
  execSync('git branch -M main', { cwd: tempRepo });
    ...
    responses: {
      create: async (payload: any) => {
        ...
        if (fakeOpenAI.calls.length === 1) {
          return {
            output: [
              {
                type: 'function_call',
                id: 'fc_missing_reasoning_1',
                call_id: 'call-missing-reasoning-1',
                name: 'read_file',
                arguments: JSON.stringify({ path: 'README.md' }),
              },
            ],
        ...
        if (fakeOpenAI.calls.length === 2) {
          throw new Error(
            "400 Item 'fc_missing_reasoning_1' of type 'function_call' was provided without its required 'reasoning' item: 'rs_missing_reasoning_1'.",
          );
        }
        return {
          output: [
  ...
  try {
    const processor = new SandboxJobProcessor(undefined, 'gpt-5-codex', fakeOpenAI);
    const job: SandboxJob = {
      jobId: 'job-provider-missing-reasoning',
      repoUrl: tempRepo,
      branch: 'main',
      taskDescription: 'trigger provider-side reasoning error and recover',
      status: 'PENDING',
      logs: [],
      interactions: [],
      interactionSequence: 0,
    ...
    assert.equal(fakeOpenAI.calls.length, 3, 'deve reenviar a requisição após o reparo');
    const retriedCall = fakeOpenAI.calls[2];
    const functionCall = retriedCall.input.find((msg: any) => msg.type === 'function_call' && msg.call_id === 'call-missing-reasoning-1');
    const functionOutput = retriedCall.input.find((msg: any) => msg.type === 'function_call_output' && msg.call_id === 'call-missing-reasoning-1');
    assert.equal(functionCall, undefined, 'function_call referenciado no erro deve ser removido no retry');
    assert.equal(functionOutput, undefined, 'function_call_output pareado também deve ser removido no retry');
  } finally {
    await fs.rm(tempRepo, { recursive: true, force: true });
  }
});
