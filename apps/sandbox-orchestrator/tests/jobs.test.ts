...
function assertGithubDeliveryInstruction(prompt: string) {
  ...
  assert.match(prompt, /corrija na sandbox e valide os ajustes/);
  assert.match(prompt, /se já houve merge, abra um PR de correção/);
  assert.match(prompt, /Não finalize como concluído enquanto houver workflow ou deploy esperado ausente/);
  assert.match(prompt, /versão\/saúde publicada verificada/);
  assert.match(prompt, /análises sem alterações não exigem PR/);
  assert.match(prompt, /restrições explícitas do usuário/);
  assert.match(prompt, /informe o bloqueio.*sem afirmar conclusão/);
  assert.doesNotMatch(prompt, /apenas não abra nem publique o PR até o usuário pedir|Pull Request executado pelo usuário|não crie nem publique PR quando/);
  assert.doesNotMatch(prompt, /deixe a mudança pronta para aguardar o deploy|avise o usu[aá]rio e aguarde o deploy/);
}
...
test('executa CHATGPT_CODEX_MKT via Codex App Server com instruções de marketing', async () => {
  ...
  try {
    ...
    assert.ok(input?.[0]?.text?.includes('quando a solicitação for criar um artefato dentro do Marketing Hub'));
    assert.ok(input?.[0]?.text?.includes('se o front-end ainda não tiver a funcionalidade necessária'));
    assert.ok(input?.[0]?.text?.includes('conclua o PR, merge e deploy conforme a orientação de entrega'));
    assert.ok(input?.[0]?.text?.includes('Quando a solicitação for alterar uma funcionalidade de módulo'));
    assert.ok(input?.[0]?.text?.includes('Nunca use SSH para publicar diretamente uma alteração'));
    ...
    assert.ok(input?.[0]?.text?.includes('somente então consolide a solução validada para publicação'));
    assert.ok(input?.[0]?.text?.includes('Execute uma rodada local dos testes relevantes'));
    assert.ok(input?.[0]?.text?.includes('repita apenas as validações necessárias para confirmar a correção'));
    assert.ok(input?.[0]?.text?.includes('Não repita toda a matriz apenas para atingir uma quantidade mínima'));
    assert.ok(!input?.[0]?.text?.includes('duas rodadas'));
...
test('inclui checklist de ambiente OK no prompt inicial do runner', async () => {
  ...
  try {
    ...
    assert.match(promptText, /somente então consolide a solução validada para publicação/i);
    assert.match(promptText, /Execute uma rodada local dos testes relevantes/i);
    assert.match(promptText, /repita apenas as validações necessárias para confirmar a correção/i);
    assert.match(promptText, /Não repita toda a matriz apenas para atingir uma quantidade mínima/i);
    assert.doesNotMatch(promptText, /duas rodadas/i);
    assert.match(promptText, /não avance para PR, merge ou deploy enquanto algum critério local de aceite estiver pendente/i);
...
test('executa CHATGPT_CODEX_SANDBOX via Codex App Server sem clonar repositório', async () => {
  ...
  assert.match(input?.[0]?.text ?? '', /sem PR, merge ou deploy/);
  assert.doesNotMatch(input?.[0]?.text ?? '', /Orientação de entrega para tarefas com código em repositório/);
  assert.ok(input?.[0]?.text?.includes('Nunca use SSH para publicar diretamente uma alteração'));
  assert.ok(input?.[0]?.text?.includes('possui Playwright e @playwright/test instalados'));
  assert.ok(input?.[0]?.text?.includes('PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH'));
  assert.ok(input?.[0]?.text?.includes('A melhor opção de simulador de celular disponível na sandbox é o Playwright com emulação mobile do Chromium'));
