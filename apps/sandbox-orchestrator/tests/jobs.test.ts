...
test('compacta histórico enviado ao modelo quando ultrapassa o limite configurado', async () => {
  const originalLimit = process.env.MODEL_INPUT_MAX_CHARS;
  process.env.MODEL_INPUT_MAX_CHARS = '10000';

  const tempRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'sandbox-history-'));
  ...
  try {
    ...
    const fakeOpenAI = {
      ...
      responses: {
        create: async (payload: any) => {
          fakeOpenAI.calls.push(JSON.parse(JSON.stringify(payload)));
          if (fakeOpenAI.calls.length === 1) {
            return {
              output: [
                {
                  type: 'function_call',
                  call_id: 'call-big-output',
                  name: 'run_shell',
                ...
                { type: 'message', id: 'msg-loop', role: 'assistant', status: 'in_progress', content: [] },
              ],
            };
          }
          return {
    ...
    assert.equal(fakeOpenAI.calls.length, 2, 'esperava pelo menos duas chamadas ao modelo');
    const secondCall = fakeOpenAI.calls[1];
    const summaryMessage = secondCall.input.find(
      (msg: any) => msg.role === 'system' && typeof msg.id === 'string' && msg.id.startsWith('msg_history_'),
    );
    assert.ok(summaryMessage, 'deveria injetar uma mensagem de resumo ao compactar o histórico');
    const summaryText = summaryMessage?.content?.find((item: any) => item.type === 'input_text')?.text ?? '';
    assert.match(summaryText, /Resumo de interações anteriores/, 'texto do resumo deveria explicar o motivo');

    assert.ok(
      job.logs.some((entry) => entry.includes('histórico enviado ao modelo excedeu')),
      'deveria registrar no log a compactação do histórico',
    );
  } finally {
    if (originalLimit === undefined) {
      delete process.env.MODEL_INPUT_MAX_CHARS;
    } else {
      process.env.MODEL_INPUT_MAX_CHARS = originalLimit;
    }
    await fs.rm(tempRepo, { recursive: true, force: true });
  }
});
