...
- O Docker CLI e o plugin Docker Compose v2 vêm pré-instalados, e o `sandbox-orchestrator` usa `DOCKER_HOST` para acessar a engine dedicada `sandbox-docker`. Essa engine não compartilha o socket do daemon de produção, recebe apenas o workspace no mesmo caminho e usa armazenamento próprio. Cada job recebe um projeto Compose determinístico; ao final, o orquestrador remove os recursos rotulados desse projeto e só executa a limpeza global de volumes sem uso quando não há outra homologação ativa, evitando tanto acúmulo quanto interferência entre jobs concorrentes.
...
- O acesso SSH persistente usa o sidecar sem rede `sandbox-ssh-agent`. Apenas ele monta `${SANDBOX_SSH_KEY_HOST_DIR:-/root/infra/sandbox-ssh}/id_ed25519`; o orquestrador recebe somente o socket Unix read-only. A identidade é carregada com restrições OpenSSH por destino e host key, e o helper `sandbox-ssh` aplica allowlist, autenticação não interativa e bloqueio de forwarding/PTY. A ausência da chave mantém o agente vazio e o acesso falha fechado sem derrubar as funções não relacionadas do Marketing Hub.
- As credenciais Luma, Kling, HeyGen, Radar Meta e Meta podem ser fornecidas por arquivos fora do repositório em `${LUMA_TOKEN_HOST_DIR:-/root/infra/luma-token}/luma_api_key`, `${KLING_TOKEN_HOST_DIR:-/root/infra/kling-token}/kling_api_key`, `${HEYGEN_TOKEN_HOST_DIR:-/root/infra/heygen-token}/heygen_api_key`, `${RADAR_META_TOKEN_HOST_DIR:-/root/infra/radarmeta-token}/radar_meta_token` e `${META_TOKEN_HOST_DIR:-/root/infra/meta-token}/meta_token`. O `docker-compose` monta esses diretórios em `/run/secrets/luma-token:ro`, `/run/secrets/kling-token:ro`, `/run/secrets/heygen-token:ro`, `/run/secrets/radarmeta-token:ro` e `/run/secrets/meta-token:ro` e exporta `LUMA_API_KEY`, `KLING_API_KEY`, `HEYGEN_API_KEY`, `RADAR_META_TOKEN` e `META_TOKEN` para o runner e para o Codex App Server quando os arquivos existem, sem versionar os segredos.
...
- O `shellcheck` vem pré-instalado para análise estática de scripts shell. A validação `bash .github/scripts/validate-shell-scripts.sh` combina `bash -n` com `shellcheck -x` nos scripts shell versionados de primeira parte e exclui código de terceiros/vendor.
