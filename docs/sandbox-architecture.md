...
- O Docker CLI e o plugin Docker Compose v2 vêm pré-instalados, e o `sandbox-orchestrator` usa `DOCKER_HOST` para acessar a engine dedicada `sandbox-docker`. Essa engine não compartilha o socket do daemon de produção, recebe apenas o workspace no mesmo caminho e usa armazenamento próprio. Use `docker compose` em vez do binário legado `docker-compose`, um nome de projeto exclusivo por homologação e remova containers, redes e volumes temporários ao terminar.
