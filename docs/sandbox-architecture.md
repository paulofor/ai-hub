...
- As credenciais Luma, Kling, HeyGen e Radar Meta podem ser fornecidas por arquivos fora do repositório em `${LUMA_TOKEN_HOST_DIR:-/root/infra/luma-token}/luma_api_key`, `${KLING_TOKEN_HOST_DIR:-/root/infra/kling-token}/kling_api_key`, `${HEYGEN_TOKEN_HOST_DIR:-/root/infra/heygen-token}/heygen_api_key` e `${RADAR_META_TOKEN_HOST_DIR:-/root/infra/radarmeta-token}/radar_meta_token`. O `docker-compose` monta esses diretórios em `/run/secrets/luma-token:ro`, `/run/secrets/kling-token:ro`, `/run/secrets/heygen-token:ro` e `/run/secrets/radarmeta-token:ro` e exporta `LUMA_API_KEY`, `KLING_API_KEY`, `HEYGEN_API_KEY` e `RADAR_META_TOKEN` para o runner e para o Codex App Server quando os arquivos existem, sem versionar os segredos.
