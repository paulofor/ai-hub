...
- As credenciais Luma e Kling podem ser fornecidas por arquivos fora do repositório em `${LUMA_TOKEN_HOST_DIR:-/root/infra/luma-token}/luma_api_key` e `${KLING_TOKEN_HOST_DIR:-/root/infra/kling-token}/kling_api_key`. O `docker-compose` monta esses diretórios em `/run/secrets/luma-token:ro` e `/run/secrets/kling-token:ro` e exporta `LUMA_API_KEY` e `KLING_API_KEY` para o runner e para o Codex App Server quando os arquivos existem, sem versionar os segredos.
