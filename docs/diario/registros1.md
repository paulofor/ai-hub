...
## 2026-07-29 06:08:40 UTC - Fallback SSH no workflow principal

- Solicitacao recebida: verificar workflow ainda falhando e ajustar.
- Pergunta explicita de causa raiz: "por que esse erro aconteceu?". Resposta: o ajuste anterior padronizou os workflows de modulos do host `191.252.120.96`, mas o workflow principal `.github/workflows/ci.yml` ainda configurava SSH usando exclusivamente `secrets.VPS_SSH_KEY`; se o secret ativo do repositorio fosse `VPS_SSH_CHAVE` ou `SSH_PRIVATE_KEY`, o deploy principal continuaria falhando por chave ausente/incorreta.
- Evidencia analisada: `gh run list --limit 10` mostrou o CI mais recente da `main` com sucesso, mas o arquivo local ainda tinha configuracao antiga no step `Configure SSH`.
- Alternativas avaliadas: (1) apenas reexecutar o workflow, rapido mas nao elimina a recorrencia; (2) trocar secrets manualmente no GitHub, dependente de acesso externo e sem rastreabilidade no repositorio; (3) aplicar no `ci.yml` o mesmo fallback de secrets SSH usado nos workflows corrigidos. Escolhida a alternativa 3 por resolver a causa raiz versionando a tolerancia aos nomes de secret existentes.
- Ajuste aplicado em `.github/workflows/ci.yml`: o step `Configure SSH` agora aceita `VPS_SSH_CHAVE`, `VPS_SSH_KEY` ou `SSH_PRIVATE_KEY`, falha com mensagem explicita se todos estiverem vazios e mantem a escrita segura da chave em `~/.ssh/id_ed25519`.
- Validacoes executadas: `actionlint .github/workflows/ci.yml`; `git diff --check`.
- Nao foi criado Pull Request.
