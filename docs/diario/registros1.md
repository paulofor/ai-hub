...
## 2026-08-24 — Plano de medição e controle da memória da suíte do Marketing Hub

- Solicitação recebida: consultar a solicitação Codex #2369 e detalhar como implementar a recomendação de executar a suíte em grupos sequenciais, limitar JVMs, medir pico de RAM e identificar retenção de contextos Spring.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. A evidência confirma pressão de memória no cgroup de 8 GiB e histórico de OOM por Java, mas ainda não permite separar heap legítimo, contextos Spring, forks concorrentes, processos órfãos e consumo fixo das demais ferramentas. Por isso, aumentar RAM isoladamente foi rejeitado como correção inicial.
- Proposta registrada: inventário determinístico dos testes e assinaturas de contexto; quatro grupos em processos Maven separados e sequenciais; limites distintos para Maven e Surefire/Failsafe; coleta de `memory.peak`, `memory.events`, RSS da árvore de processos, GC e heap dump apenas em falha; e estatísticas do cache Spring para priorizar substituição de `@SpringBootTest`/`@DirtiesContext` quando a evidência indicar.
- Implantação em fases: primeiro cinco baselines com gate informativo, depois correção das classes responsáveis e finalmente gates obrigatórios de OOM, sequencialidade e pico abaixo de 75% do limite efetivo, sem reduzir testes ou cobertura. Um aumento para 12 GiB fica condicionado à medição e ao aumento real da capacidade do host.

## 2026-08-24 — Orientação de controle de memória incorporada ao modelo

- Solicitação recebida: implementar imediatamente para o modelo a orientação definida a partir da solicitação #2369, em vez de manter somente o documento de proposta.
- Pergunta explícita de causa raiz: “por que a orientação anterior não mudava o comportamento do modelo?”. Ela existia apenas em documentação; o prompt operacional continuava exigindo validação local completa sem ensinar como reagir quando uma suíte JVM monolítica disputasse o limite do cgroup. Assim, o modelo ainda podia repetir a suíte pesada ou sugerir mais RAM sem atribuir o pico.
- Correção na causa: a instrução obrigatória compartilhada pelos caminhos Codex App Server e Responses API agora manda medir limite e baseline, executar todos os testes em grupos estáveis e sequenciais com uma JVM por vez, limitar JVM de build e forks, registrar pico/eventos de OOM/processos e investigar cache Spring e `@DirtiesContext`, sem reduzir cobertura nem publicar heap dumps.
- Proteção contra regressão: os testes dos dois caminhos de inferência verificam que a orientação completa chega efetivamente ao prompt do modelo, incluindo sequencialidade, observabilidade, preservação da cobertura e segurança dos diagnósticos.
