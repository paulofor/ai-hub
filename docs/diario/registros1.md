...
## 2026-07-31 18:29 UTC - Nova verificação de aparente travamento

- Solicitação recebida: verificar novamente o que aconteceu e se o AI Hub havia travado.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. Nesta verificação não foi encontrado um travamento ativo. A pilha inteira havia sido recriada às `18:21 UTC` com a imagem do commit `f293321`, e os eventos Docker identificam os containers com o atributo `com.docker.compose.replace`, evidência de execução de `docker compose up`/deploy, não de reinício automático causado por falha. Todos estavam ativos, com `RestartCount=0`, `OOMKilled=false` e sem término registrado.
- A sensação de travamento logo após essa recriação é compatível com a janela de inicialização: o Caddy começou a encaminhar sondagens imediatamente, enquanto o backend Spring levou aproximadamente 36 segundos para concluir o startup, incluindo conexão ao MySQL e validação das 39 migrations. As primeiras sondagens internas do backend e MCP falharam enquanto os processos ainda subiam.
- Estado confirmado ao final: `https://iahub.xyz/` respondeu `HTTP 200` em aproximadamente 225 ms e `https://iahub.xyz/mcp` respondeu `HTTP 200` com `{"status":"UP"}` em aproximadamente 167 ms. O host tinha 4,3 GiB de memória disponível, disco em 37% e load average `0.95/0.84/0.63`; não há evidência de OOM, disco cheio ou sobrecarga.
- Os logs posteriores mostram operação normal, inclusive criação e despacho da solicitação Codex 897. Houve avisos isolados de `Broken pipe`, causados por clientes que fecharam a conexão, mas eles não derrubaram o backend.
- Nenhuma reinicialização ou ação destrutiva foi executada. Limitação: o cliente `gh` não está instalado neste ambiente, portanto não foi possível correlacionar diretamente o horário da recriação com a execução específica do GitHub Actions; a identificação como deploy é baseada nos eventos e metadados do Docker Compose.
