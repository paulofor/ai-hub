...
## 2026-08-03 17:00:50 UTC-3 — Diagnóstico de efetividade comercial do Codex ChatGPT MKT

- Solicitação recebida: acessar o histórico de solicitações do ambiente, explicar por que o Marketing Hub ainda não gera vendas e orientar como o perfil Codex ChatGPT MKT pode atuar com mais efetividade, inclusive avaliando aumento de autonomia.
- Pergunta explícita de causa raiz: “por que esse problema aconteceu?”. Resposta: o perfil já possui autonomia técnica alta e grande volume de execução, mas o sistema mede e otimiza atividade interna (solicitações, interações, PRs e impacto comercial autodeclarado pelo modelo), sem usar receita e eventos reais do funil como fonte de verdade para priorização e aprendizado. Assim, muito trabalho pode terminar em software, páginas, análises e handoffs tecnicamente concluídos sem fechar o ciclo tráfego → checkout → compra → entrega → satisfação.
- Evidências consultadas em modo somente leitura: histórico público da API do AI Hub, detalhes das 120 solicitações MKT mais recentes, métricas operacionais do perfil, prompt atual do modo MKT, implementação do frontend/sandbox-orchestrator e registros anteriores do diário.
- Evidências quantitativas: 1.109 solicitações MKT entre 23/07 e 03/08; 1.076 concluídas, 12 falhas, 20 cancelamentos e 1 em execução; apenas 1 solicitação avaliada pelo usuário; 1.067 registros associados a URL de PR. Nas 120 solicitações recentes, 82 eram retomadas pós-PR, 22 eram comandos para executar orientação anterior e 33 mencionavam diretamente produto, página de vendas, campanha, anúncio, checkout ou venda.
- Métricas atuais mostram intensidade, não resultado: em 03/08 havia 35 solicitações, 23.391 interações e mais de 5 horas de processamento; o placar classificou 30 de 33 respostas como impacto `alto` ou `muito_alto`, mas essa classificação vem da própria resposta do modelo e não comprova vendas.
- Padrão comercial recente identificado: houve trabalho relevante no Agenda Cheia Nail Design (página, checkout, preço de teste, compra, pós-compra, imagens e campanha) e recuperação do MUSA, porém o histórico continua fragmentado em muitos ciclos de implementação/deploy/continuação e não apresenta um painel canônico de receita, conversão, CAC, entrega, reembolso e satisfação que governe a próxima ação.
- Recomendação: não aumentar autonomia de forma irrestrita. Adotar autonomia orientada por meta e orçamento: o modelo pode investigar, criar/editar artefatos pelo frontend, implementar e testar código, acompanhar deploys, executar QA, propor e preparar experimentos; ações com gasto, ativação de campanha, mudança de preço público, envio em massa, publicação e PR continuam com limites e/ou confirmação humana.
- Próxima evolução proposta: criar um modo operacional de crescimento com um produto prioritário por vez, placar baseado em eventos reais, backlog ranqueado por gargalo de receita, definição obrigatória de hipótese/métrica/limite antes de cada experimento e retrospectiva automática baseada em vendas e entrega.
- Nenhum código de funcionalidade foi alterado e nenhum Pull Request foi criado; apenas este registro obrigatório foi acrescentado ao diário.

## 2026-08-03 20:15 UTC — Implementação do modo Operador de Crescimento

- Solicitação recebida: implementar a recomendação do diagnóstico comercial anterior, tornando vendas e eventos reais do funil a fonte de verdade do perfil Codex ChatGPT MKT.
- Pergunta explícita de causa raiz: “por que o perfil executava muito sem comprovar vendas?”. Resposta: o sistema persistia solicitações, interações, PRs e uma nota de impacto comercial autodeclarada pelo modelo, mas não persistia uma missão comercial, limite de gasto nem os eventos visitante → CTA → checkout → venda → briefing → entrega/reembolso. Sem esse estado canônico, cada nova solicitação podia otimizar atividade local em vez do maior gargalo de receita.
- Correção na causa: criada a entidade persistente `growth_missions`, com API para consultar/atualizar uma missão, produto prioritário, objetivo, meta de vendas, prazo, status, limite de gasto, visitantes, cliques, checkouts, vendas aprovadas, briefings, entregas, reembolsos, receita e gasto.
- Governança automática: o backend calcula conversão, CAC e o gargalo atual (`INSTRUMENTACAO`, `OFERTA`, `CHECKOUT`, `PAGAMENTO`, `ENTREGA`, `TRAFEGO`, `ORCAMENTO_ESGOTADO` ou `META_ATINGIDA`), bloqueia métricas incoerentes e recomenda a próxima decisão. Orçamento esgotado e entrega atrasada impedem que a recomendação padrão seja simplesmente comprar mais tráfego.
- Interface: a tela Codex ChatGPT MKT ganhou o painel “Operador de Crescimento” para editar a missão e o placar real. O antigo “Placar de vendas”, que mede somente impacto potencial declarado pelo modelo, passou a se chamar “Impacto estimado”.
- Comportamento do agente: cada solicitação MKT recebe automaticamente o estado comercial e regras de autonomia controlada. A injeção ocorre no frontend e também no backend, protegendo chamadas feitas diretamente à API. O modelo deve declarar gargalo, evidência, métrica e critério de decisão; não pode confundir PR/atividade com venda nem ultrapassar orçamento, publicar, alterar preço/campanha ou enviar em massa sem autorização.
...
## 2026-08-03 20:31 UTC — Automação do placar do Operador de Crescimento

- Solicitação: eliminar a intervenção manual na atualização do funil e permitir que o perfil MKT trabalhe com dados comerciais reais automaticamente.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. Resposta: a V42 persistia totais editáveis na própria missão porque o AI Hub não possuía entidades de vendas/checkout/campanhas nem um contrato de integração com os sistemas que originam esses dados. Sem uma entrada de eventos, a interface só podia funcionar como planilha manual.
- Solução: criada a V43 `growth_events` para MySQL, PostgreSQL e H2, com eventos imutáveis ligados à missão e chave idempotente `source + external_id`. Criado endpoint autenticado `POST /api/growth/events`, protegido por `HUB_GROWTH_EVENTS_TOKEN`, validação de tipo/produto e tratamento seguro de reenvios concorrentes.
- Eventos aceitos: visita, clique no CTA, checkout iniciado, venda aprovada, briefing concluído, entrega concluída, reembolso e gasto de mídia. Receita, gasto, CAC, conversão, gargalo e fusíveis passam a ser agregados automaticamente desses eventos.
- Interface: campos de funil tornaram-se somente leitura e o painel informa se está aguardando integração ou usando eventos automáticos, quantidade recebida e métricas calculadas. O contexto entregue ao perfil MKT proíbe pedir preenchimento manual quando faltarem eventos e orienta investigar a instrumentação.
- Segurança/operação: o segredo não é exposto ao navegador; produtores server-side usam o contrato documentado em `docs/marketing/growth-events.md`. Este repositório implementa o receptor. A emissão pelos sistemas de Marketing Hub, checkout e mídia deve ser versionada nos respectivos repositórios quando eles estiverem no ambiente do modelo.
- Validação: 108 testes backend aprovados; build e lint frontend aprovados; aplicação iniciada em H2 isolado com todas as migrations até V43; fluxo HTTP real validou criação da missão, venda de R$ 97, rejeição idempotente do reenvio e atualização automática do gargalo para entrega; `git diff --check` aprovado.
- Compatibilidade MySQL 5.7: a primeira inicialização herdou inadvertidamente o `DB_URL` externo do ambiente e tentou V43. O `CREATE TABLE` foi rejeitado antes de criar a tabela por `TIMESTAMP(6) NOT NULL` sem default; o Flyway registrou tentativa falha, que a estratégia `repair-on-startup` existente remove automaticamente na próxima inicialização. A migration foi corrigida para `DATETIME(6)`. O teste completo do arquivo MySQL 5.7 ficará a cargo do workflow `liquibase-mysql57.yml` no PR, conforme política do projeto; nenhum deploy ou PR foi realizado.

## 2026-08-03 21:05 UTC — Redesign do Operador de Crescimento

- Solicitação recebida: corrigir o visual do painel a partir da captura da interface publicada.
- Pergunta explícita de causa raiz: “por que o visual ficou ruim?”. Resposta: configuração, métricas automáticas e recomendação estavam apresentadas como onze campos de formulário com o mesmo peso visual; o card operacional fixo sobrepunha o conteúdo; e o shell reservava 240 px para o menu mesmo em celular, comprimindo toda a aplicação e provocando overflow horizontal.
- Redesign: a recomendação e o gargalo passaram a ocupar a área de maior destaque; vendas, receita, conversão e CAC ganharam placar próprio; o funil virou uma sequência visual; meta, orçamento, pós-venda e saúde receberam agrupamentos semânticos; e os poucos campos realmente editáveis ficaram em “Configurar missão”, recolhido por padrão quando já existe missão.
...
## 2026-08-05 — Diagnóstico da falha no GitHub Actions após o merge do PR #627

- Solicitação recebida: identificar o motivo do erro no GitHub Actions.
- Execução investigada: workflow `CI`, run `31013259550`, disparado por push na `main` para o commit de merge `a4393b14345ae01645e58e2712ebdc7f1eb00984`.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. Resposta: o GitHub Container Registry aceitou a autenticação, compilou a imagem do frontend e publicou a tag `latest`, mas recusou temporariamente a publicação da segunda tag (SHA do commit) com HTTP 403 porque a conta/execução excedeu um limite secundário do GitHub. A própria resposta solicitou aguardar alguns minutos antes de tentar novamente.
- Evidências: os jobs `frontend`, `backend`, `mcp-server` e `sandbox` passaram; no job `docker`, as imagens de Caddy e backend passaram; somente `Build and push frontend image` falhou. O log mostra o build integralmente em cache e concluído, o push de `latest` concluído e a recusa apenas de `ghcr.io/paulofor/ai-hub-6-frontend:a4393b...`. O job `deploy` foi ignorado por depender do sucesso do job `docker`.
- Conclusão: falha transitória de rate limit da infraestrutura do GitHub/GHCR, sem evidência de defeito no código, nas credenciais do registry ou no token da Meta. Nenhuma correção de código foi aplicada; a ação imediata indicada é reexecutar o run após o período de espera.
...
## 2026-08-07 — Validação local obrigatória antes de publicar em todos os perfis

- Solicitação recebida: orientar o modelo, em todos os perfis, a concluir testes e ajustes no próprio ambiente antes de enviar código ao repositório e iniciar deploy/publicação.
...
## 2026-08-29 — Correção da colisão Flyway que quebrou o CI do AI Hub

- Solicitação recebida: investigar e ajustar o erro do GitHub Action do AI Hub, deixando a correção validada localmente para uma nova execução posterior.
- Gargalo declarado antes da execução: pipeline de publicação bloqueado. Evidência inicial: run de PR `33269104038` concluído com falha no job `backend`; métrica esperada: catálogo Flyway válido e toda a matriz local verde; critério: continuar diante de falha reproduzível, ajustar a cada novo defeito e não publicar diante de dependência externa ou teste pendente.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. Resposta: a branch da recuperação criou `V44__create_public_proxy_recoveries.sql` enquanto a `main` já havia recebido `V44__add_codex_reasoning_effort.sql` e depois `V45__index_codex_requests_status_created_at.sql`. Como os nomes completos eram diferentes, o merge Git não gerou conflito, mas o Flyway exige uma única migration por versão. O PR foi mergeado às `18:46:16Z`, antes de seu CI terminar com falha às `18:46:32Z`, e não existia uma verificação estática rápida das versões. A execução de `main` `33269149840` repetiu a falha; seu job Docker, sem dependência dos testes, ainda publicou as imagens e tags mutáveis, embora o deploy tenha sido corretamente ignorado.
- Alternativas avaliadas: (1) apenas renomear a nova migration para `V46`, com baixo esforço, mas sem prevenção da recorrência; (2) renumerar as migrations antigas, com risco alto por alterar histórico já potencialmente aplicado; (3) mover somente a migration inédita para `V46`, adicionar validação estática de unicidade para H2/MySQL/PostgreSQL e fazer a publicação Docker depender de toda a matriz de testes; escolhida a terceira por preservar o histórico e corrigir também a falha de contenção.
- Ajustes aplicados: a criação de `public_proxy_recoveries` passou de `V44` para `V46` nos três bancos; referências do teste H2 e do ensaio MySQL/PostgreSQL foram atualizadas; foi criado `validate-migration-versions.sh`, executado antes dos testes de banco e capaz de apontar os dois arquivos conflitantes; o job `docker` passou a depender de `backend`, `frontend`, `sandbox` e `mcp-server`, impedindo novas imagens de revisão não validada.
- Proteção contra regressão: o catálogo corrigido foi aceito; uma fixture negativa com duas migrations `V1` foi rejeitada; `actionlint`, `bash -n` e `git diff --check` passaram. A migration completa chegou à versão `46` em H2 e criou a tabela com unicidade por `request_id`; MySQL 5.7 e PostgreSQL aplicaram o SQL real e rejeitaram UUID duplicado.
...
## 2026-08-31 — Registro oficial OpenAI para indisponibilidade de capacidade do Codex

- Solicitação recebida: verificar se existe registro de problema da OpenAI compatível com o erro `Selected model is at capacity. Please try a different model.` que interrompeu as solicitações 2544 e 2547.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. A evidência histórica confirma que a mensagem pode decorrer de indisponibilidade temporária de capacidade no provedor, e não apenas de limite de uso, erro do GitHub Actions ou falha do repositório.
- Evidência oficial: o histórico público da OpenAI contém o incidente `Codex "Selected Model is at Capacity" Error`, iniciado em 16/06/2026 às 10:32 (horário exibido na página), com mitigação aplicada e resolução às 13:48; afetou ChatGPT e quatro componentes Codex. Há também o incidente `Codex 5.6-sol Experiencing Increased Server-Overload Errors`, em 17/07/2026, igualmente resolvido.
- Estado no momento da consulta: a página de status da OpenAI informa operação normal e ausência de problemas conhecidos. O histórico consultado não mostra incidente público correspondente aos intervalos das solicitações 2544 e 2547; isso não exclui impacto pontual por modelo, tier ou recurso, pois a própria página informa que suas métricas são agregadas.
- Alternativas avaliadas: (1) atribuir automaticamente o erro a limite da conta, simples mas incompatível com os incidentes oficiais; (2) classificá-lo automaticamente como outage geral, também incorreto sem incidente correlacionado no horário; (3) classificá-lo como indisponibilidade de capacidade do provedor com correlação externa inconclusiva, preservando a mensagem original e tentando novamente depois ou com modelo disponível alternativo. Escolhida a alternativa 3 por refletir a evidência sem extrapolação.
- Estado externo preservado: nenhuma tentativa, PR, deploy, campanha, gasto ou venda foi criada nesta consulta.

## 2026-09-06 — Desenho para persistência protegida do acesso SSH da sandbox

- Solicitação recebida: explicar como persistir e disponibilizar uma chave SSH operacional entre execuções efêmeras da sandbox, sem expor a chave privada na conversa ou no repositório.
- Gargalo real: acesso operacional aos hosts não sobrevive entre execuções. Evidência: a sandbox atual contém somente `known_hosts`; o `docker-compose.yml` monta segredos persistentes para AWS, GitHub e APIs, mas não possui mount, socket ou serviço dedicado para SSH.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. Resposta: a chave anterior foi gerada dentro de uma sandbox efêmera. A chave pública permaneceu nos `authorized_keys` dos hosts, mas a chave privada não foi persistida fora do ciclo de vida do job e, portanto, não está disponível nesta nova execução.
- Alternativas avaliadas: (1) montar a chave privada diretamente no `sandbox-orchestrator`, com baixo esforço, mas permitindo que processos do agente leiam o material secreto; (2) manter a chave em diretório protegido do host, carregá-la em um `ssh-agent` auxiliar e montar somente o socket no orquestrador, com esforço moderado e melhor isolamento; (3) emitir certificados SSH de curta duração por cofre/CA a cada job, com melhor rotação e revogação, porém maior custo operacional e dependências novas. Escolhida a alternativa 2 por aderir ao Compose e ao padrão de segredos existentes sem expor o arquivo privado ao agente.
- Contrato recomendado: segredo fora do repositório em diretório `0700`, chave `0600`, `ssh-agent` como único consumidor do arquivo, socket Unix compartilhado com o orquestrador, `known_hosts` fixado, destinos e usuários em allowlist, `BatchMode=yes`, encaminhamentos e PTY desabilitados, usuário remoto dedicado com `sudo` mínimo e nenhuma impressão do segredo em logs.
- Métrica e critério: 100% das novas execuções devem autenticar com `ssh -o BatchMode=yes` nos hosts autorizados usando o mesmo fingerprint, sem chave privada no workspace, ambiente, patch ou logs. Continuar após validar todos os destinos; ajustar diante de host key ou permissão divergente; revogar imediatamente diante de fingerprint inesperado, destino fora da allowlist ou indício de exposição.
- A documentação oficial do Codex Cloud foi consultada: segredos configurados no ambiente são removidos antes da fase do agente, portanto não resolvem sozinhos o uso operacional de SSH; a persistência deve ser responsabilidade explícita do orquestrador.
- Nenhuma chave foi gerada, copiada ou exibida; nenhum host, PR, deploy, campanha, gasto ou venda foi alterado nesta análise.

## 2026-09-07 — Implementação do acesso SSH persistente e protegido da sandbox

- Solicitação recebida: implementar a identidade SSH permanente entre execuções e entregar somente a chave pública para cadastro nos hosts.
- Gargalo real: a sandbox não possuía identidade operacional reutilizável. Evidência inicial: a chave anterior foi criada dentro de uma execução efêmera e desapareceu com ela; o Compose não possuía serviço de `ssh-agent`, socket compartilhado ou provisionamento persistente. Métrica esperada: 100% das novas execuções enxergarem o mesmo fingerprint nos quatro destinos autorizados, com zero bytes privados no workspace, imagem, ambiente, logs ou diff. Critério: continuar com fingerprint estável e bloqueio de destinos indevidos; ajustar diante de prontidão, permissão ou host key incorreta; revogar diante de identidade inesperada ou exposição.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. Resposta: a chave privada tinha o mesmo ciclo de vida curto da sandbox, enquanto apenas a chave pública permanecia nos VPS. Não havia um componente persistente responsável por guardar a identidade fora do job e oferecê-la sem expor o arquivo ao modelo.
- Alternativas avaliadas: (1) montar a chave privada diretamente no orquestrador, simples porém legível pelo agente; (2) gerar uma chave nova a cada job, segura temporalmente porém incompatível com permanência; (3) manter o segredo fora do repositório, carregá-lo em sidecar `ssh-agent` sem rede e montar somente o socket no orquestrador. Escolhida a alternativa 3 pelo equilíbrio entre isolamento, auditabilidade, esforço e aderência ao objetivo.
- Implementação: nova identidade Ed25519; chave pública e fingerprint versionados; chave privada armazenada no segredo protegido `SANDBOX_OPS_SSH_PRIVATE_KEY`; workflow com validação criptográfica e provisionamento atômico em `/root/infra/sandbox-ssh/id_ed25519` modo `0600`; host keys dos quatro VPS fixadas; `StrictHostKeyChecking=yes`; sidecar sem rede, read-only, sem capabilities e com limite de memória/processos; socket somente leitura no orquestrador; restrições OpenSSH por destino; helper `sandbox-ssh` fail-closed; instrução operacional ao modelo, documentação e testes de regressão.
- Defeitos iterativos e causas: o agente daemonizado não podia ser aguardado pelo entrypoint e passou a rodar em foreground; o socket indicava prontidão antes da carga e ganhou marcador posterior; o deploy aceitava host key desconhecida e passou a usar as chaves fixadas; a comparação da chave pública incluía comentário variável e passou a normalizar algoritmo e material; dois falsos positivos do harness foram corrigidos sem reduzir a detecção.
- Homologação final: após a última correção, duas rodadas locais completas e consecutivas passaram. Em cada uma foram aprovados 94 testes do orquestrador, ShellCheck, Actionlint, contratos Compose, `git diff --check`, build da imagem e ciclo Docker real com agente vazio, carga segregada, ausência da chave no orquestrador, destino indevido bloqueado, assinatura genérica recusada e fingerprint preservado após reinício.
- Segregação e limpeza: toda a topologia Docker usou o projeto `aihub-21e542d9-4c3b-44e5-a09b-a3960e56455b-547f87561f`; a chave privada temporária foi destruída depois da gravação protegida; a imagem local de homologação foi removida; restaram zero containers e volumes do projeto.
- Estado externo: foi criado apenas o segredo protegido necessário no GitHub Actions, cujo valor não é recuperável nem foi impresso. Nenhum commit, PR, push, workflow, deploy, host, campanha, gasto ou venda foi alterado. A autenticação real fica pendente do cadastro da chave pública pelo proprietário e da publicação normal da implementação por PR.

## 2026-09-07 — Extensão da identidade SSH para homologação Docker remota

- Solicitação recebida: garantir que a identidade SSH permanente também permita ao modelo transmitir imagens Docker e instalar containers temporários para teste e depuração, entregando a chave pública para cadastro nos hosts.
- Gargalo real declarado: a identidade persistente e seu segredo protegido já existiam, mas a sandbox possuía somente um shell SSH genérico e nenhum ciclo segregado de imagem/container. Evidências: `SANDBOX_OPS_SSH_PRIVATE_KEY` está cadastrado no GitHub e a pública versionada possui fingerprint `SHA256:NJ2GkGnHsfNjeA9FDUoL+PQLHPCB9JNnysoonLtjvkc`; a sandbox atual não possui `SSH_AUTH_SOCK` porque a versão do sidecar ainda não foi publicada. Métrica: ciclo completo de imagem/container com mesmo hash, zero tar remoto, zero interferência entre sessões e zero resíduo.
- Pergunta explícita de causa raiz: “por que a chave permanente, sozinha, não conclui esse fluxo?”. Resposta: autenticação não fornece governança do Docker remoto. Sem namespace, labels, limites e limpeza, comandos livres como `root` podem consumir disco ou atingir recursos publicados por engano.
- Alternativas avaliadas: (1) `scp` de tar, simples mas com duplicação e resíduos; (2) contexto Docker remoto irrestrito, flexível mas com alta chance de colisão produtiva; (3) helper sobre o SSH protegido, usando stream e ciclo temporário segregado. Escolhida a alternativa 3 pelo equilíbrio entre capacidade operacional, segurança, custo e aderência à regra de publicação versionada.
- Implementação: criado `sandbox-remote-docker` com `push`, `run`, `exec`, `logs`, `inspect`, `remove`, `unload` e `cleanup`; transferência `docker image save | ssh docker image load`; comparação do image ID; namespace `aihubsbx/<sessão>`; containers `aihubsbx-<sessão>-<nome>`; labels de gestão/sessão; limites configuráveis de CPU, RAM e PIDs; restart desabilitado; `no-new-privileges`; remoção de capabilities; rede `none` por padrão e bloqueio de host network; ausência deliberada de mounts, socket Docker e modo privilegiado. A instrução entregue aos modelos, Dockerfile, ambiente, README, guia operacional e CI foram atualizados.
- Endurecimento adicional: `sandbox-ssh` passou a serializar cada argumento remoto como token literal, evitando que espaços, aspas ou metacaracteres alterem o comando pretendido; quebras de linha são recusadas. A pública pode ser cadastrada com o prefixo OpenSSH `restrict`, preservando comandos remotos enquanto bloqueia forwarding e PTY.
- Defeito encontrado: ShellCheck apontou a cardinalidade ambígua `A && B || C` em `logs`; a causa foi uma validação condensada e a correção usou `if` explícito. O preflight inicial também não exportava variáveis entre comandos Compose e misturava avisos ShellCheck antigos do workflow com Actionlint; o harness versionado passou a configurar o ambiente e validar as duas ferramentas separadamente.
...
## 2026-09-13 — Orientação sobre queda da nota média diária de impacto em vendas

- Solicitação recebida: sugerir como melhorar a nota média diária de vendas após queda percebida nos últimos dias.
- Pergunta de causa raiz: “por que esse erro aconteceu?”. A queda ainda não foi caracterizada como erro: a inspeção local confirmou que a nota representa impacto comercial estimado pelo modelo, e não vendas realizadas. O prompt atribui níveis menores a tarefas técnicas sem conexão comercial relevante e níveis maiores a intervenções diretas em oferta, funil e conversão.
- Evidências locais: `apps/sandbox-orchestrator/src/jobProcessor.ts` define a rubrica; `CodexRequestService.java` extrai os níveis das respostas; `CodexChatgptPage.tsx` calcula a média das notas válidas de 1 a 5; `DashboardPage.tsx` agrupa as notas por dia operacional. Cada solicitação avaliada tem o mesmo peso, independentemente do esforço ou da receita gerada.
...
## 2026-09-13 — Diagnóstico dos campos “Resumo do raciocínio” vazios

- Solicitação recebida: explicar por que os campos “Resumo do raciocínio” ficam em branco, no contexto do contrato JSON das respostas.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. A implementação de armazenamento e apresentação do resumo não habilita explicitamente sua geração. O `turn/start` envia `effort`, mas omite `summary`; o catálogo disponível nesta execução, atualizado em 2026-09-13 e identificado como Codex 0.153.4, informa `default_reasoning_summary: "none"` para `gpt-6-astra`, `gpt-5.6-sol` e `gpt-daybreak-blue-latest`. O nível de esforço e a geração do resumo são configurações independentes.
- Cuidado com a versão investigada: o checkout ativo está em `a4af342` e ainda não contém o campo. A implementação existente em `origin/main`, commit `3b27464`, contém a persistência, os eventos e a interface; foi inspecionada com `git show`, sem alterar a branch. O bundle servido por `https://iahub.xyz` contém o mesmo rótulo e lê `reasoningSummary`.
- Evidência do sistema publicado: os detalhes das solicitações concluídas 2812, 2813, 2814 e 2815 retornaram `reasoningSummary` sem conteúdo. Assim, nessas quatro amostras a ausência já existe na resposta da API, antes da renderização. Os históricos consultados não contêm eventos de resumo, mas isso não comprova ausência de emissão: `modelTranscript` persiste somente interações `OUTBOUND`, e o coletor de resumo não registra seus deltas nesse histórico.
- Caminho do dado em `origin/main`: `jobProcessor.ts` agrega `item/reasoning/summaryTextDelta` e separa partes em `summaryPartAdded`; `buildJobPayload` conserva `reasoningSummary`; `SandboxOrchestratorClient` lê o campo; `CodexRequestService.applySandboxResponseContent` persiste valores não vazios; `CodexRequestDetailPage.tsx` apresenta o valor ou “Não disponibilizado pelo modelo” e um travessão. Esse campo não vem do JSON final obrigatório do perfil MKT.
- Lacunas adicionais confirmadas no código: o coletor do App Server não aproveita `item.summary` recebido somente em `item/completed`; o caminho Responses API também envia apenas `reasoning.effort`, sem `reasoning.summary`. Os testes anteriores injetam resumos artificialmente, sem exigir que a chamada de saída solicite sua geração, e por isso não cobrem o padrão desabilitado do catálogo.
- Validação local de diagnóstico: executado o método real `runWithCodexAppServer` extraído de `origin/main`, compilado em diretório temporário, com App Server e efeitos auxiliares substituídos por doubles. Confirmados três cenários em duas execuções consecutivas: parâmetro omitido com padrão `none` mantém o campo vazio; habilitação simulada no transporte com emissão de delta preenche o campo; resumo fornecido exclusivamente no item concluído é ignorado. A resposta final normal foi preservada em todos os cenários. Os cenários demonstram o comportamento atual, não representam homologação de uma correção.
- Ajuste do harness: a instalação global de TypeScript expõe somente informações de versão via `require('typescript')`, sem a API `createSourceFile`; a primeira tentativa não executou os cenários. O diagnóstico passou a usar o compilador `tsc` disponível, sem alterar dependências do projeto, e então concluiu as verificações. Os arquivos temporários foram removidos.
- Fontes oficiais consultadas com a skill `openai-docs`: https://developers.openai.com/api/docs/guides/reasoning#reasoning-summaries (opt-in explícito e separação entre esforço e resumo); https://learn.chatgpt.com/docs/app-server#item-deltas (eventos de resumo). O schema local `exemplos/codex-rs/app-server-protocol/schema/typescript/v2/TurnStartParams.ts` também documenta a opção `summary` por turno.
- Limites e escopo: não foi feita uma nova geração com provedor real nem inspecionada a configuração privada dos processos de produção. As evidências combinam código versionado, metadados locais do catálogo, leitura dos registros publicados e simulação local. Resumos públicos disponibilizados pela API são distintos do raciocínio interno completo; a ausência de texto não significa ausência de processamento pelo modelo. Nenhuma funcionalidade, configuração de execução, registro produtivo, commit, push, PR ou deploy foi alterado; esta entrega registra o diagnóstico solicitado.

## 2026-09-13 — Correção da geração e captura dos resumos de raciocínio

- Solicitação: fazer os ajustes necessários para preencher os resumos das solicitações.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. A opção `summary` não é enviada ao provedor, cujo padrão pode ser `none`; além disso, o coletor existente na `main` só concatena deltas e perde resumos disponibilizados exclusivamente no item final. A correção deve habilitar a geração e tratar os eventos pelo identificador do item, sem duplicação e sem misturar solicitações.
- Base local: a branch recebida (`eab39a9`) antecede a implementação de persistência/interface da `origin/main` (`3b27464`). Foram transportados apenas os pré-requisitos de resumo já existentes na `main` (campo, leitura, persistência, apresentação e as mesmas migrações V49), preservando as demais funcionalidades da branch e o diário. Nenhum commit ou publicação foi necessário para montar essa base.
- Matriz definida antes dos testes: (1) opt-in `summary: auto` no App Server em todos os perfis e retomadas; (2) streaming, múltiplos itens/seções, conclusão sem deltas e conclusão duplicada; (3) ausência, payload inválido e exclusão de conteúdo bruto; (4) isolamento por thread/turno e limpeza de listeners; (5) Responses API com modelos compatíveis, chamadas de ferramenta e modelo sem suporte; (6) transporte HTTP/callback, persistência e leitura do resumo sem alterar resposta final, métricas ou registros de outra solicitação; (7) apresentação local no Chromium desktop e emulação mobile, estado vazio e atualização; (8) builds, testes de regressão pertinentes e revisão do diff. Todos os dados de teste são sintéticos, com provedores substituídos localmente e banco de teste isolado.
- Critério de homologação: reproduzir os defeitos localmente; após a última correção, concluir duas rodadas completas e consecutivas da matriz sem falhas. Geração real depende de suporte e disponibilidade do provedor e não será substituída por resumo inventado do conteúdo da resposta. Migrações idênticas às já existentes na `main`; a validação MySQL 5.7 pelo runner do PR continua no fluxo normal, caso necessária.
- Fontes de contrato: https://learn.chatgpt.com/docs/app-server (parâmetro por turno e item final autoritativo); https://developers.openai.com/api/docs/guides/reasoning#reasoning-summaries (opt-in, `auto` e resumos públicos do provedor).
- Correções implementadas: opt-in `summary: "auto"` em todo `turn/start` (incluindo retomadas e os três perfis ChatGPT Codex) e `reasoning.summary: "auto"` na Responses API para os modelos reconhecidos ou com esforço explicitamente configurado. Modelos econômicos sem raciocínio mantêm a chamada sem esse parâmetro. O novo `ReasoningSummaryCollector` agrupa por turno, item e `summaryIndex`, aceita os formatos públicos do App Server/Responses, substitui deltas pelo item final autoritativo, aproveita itens no turno concluído e evita duplicação. Conteúdo bruto e conteúdo cifrado não viram resumo.
- Defeitos relacionados reproduzidos e corrigidos: a simulação mostrou que o cliente compartilhado permitia eventos de outra thread alterarem o job, inclusive a resposta final; os listeners do job agora filtram `threadId`. O teste com o envelope oficial mostrou que `extractCodexStatus`/`extractCodexErrorMessage` ignoravam `turn.status`/`turn.error`, tratando falha como sucesso; a leitura foi corrigida para permitir a retomada e preservar os resumos das tentativas. A reprodução inicial teve sete cenários falhando antes dessas correções.
- Persistência e exibição: o backend lê `reasoningSummary`/`reasoning_summary`, guarda separadamente da resposta e conserva o texto diante de callbacks antigos/vazios. O detalhe apresenta o resumo com a normalização e o estado ausente já usados na `main`. Os três SQLs V49 são byte a byte idênticos a `origin/main`; não houve alteração de changelog Liquibase nem execução de workflow remoto.
- Ajustes do harness local: foram corrigidos a fixture sem `timeoutCount`, o nome da opção `jobRegistry`, o preflight que deve apontar à raiz Git, a leitura do envelope `interaction-audit-v1` e a configuração `browserName` do Playwright que pertence ao escopo superior do arquivo. Esses ajustes permitiram executar os testes nas interfaces reais dos módulos, sem relaxar as asserções funcionais.
- Homologação concluída: duas rodadas completas e consecutivas após a última correção, ambas com **110 testes do orquestrador, 68 testes pertinentes do backend e 5 testes E2E de interface**, todos sem falhas ou testes ignorados. Cada rodada também aprovou build TypeScript, lint/build do frontend, revisão `git diff --check` e migração V49 no MySQL 5.7 real da engine local. Os testes JDBC validaram a V49 H2 e a sintaxe da V49 PostgreSQL em H2; não foi usado um servidor PostgreSQL real.
- Evidência integrada: a fixture de processo App Server só emite resumo se a chamada habilita `auto`; o cliente JSON-RPC real o coleta; `GET /jobs/{id}` devolve o mesmo valor; o teste Spring passa esse payload pelo endpoint de callback autenticado, grava e relê pelo JPA/H2 e consulta o endpoint de detalhe; o Playwright abre o frontend local com essa mesma resposta da API. A cadeia preservou exatamente `Resumo público validado por JSON-RPC.` e manteve a resposta final em seu campo próprio. O navegador usou rotas locais simuladas, não um backend produtivo. Screenshots desktop/mobile foram inspecionadas; polling, estado vazio, formato legado e ausência de erros de página passaram.
- Dados e métricas: dados sintéticos em banco H2 exclusivo, transações de teste revertidas e MySQL efêmero; requisição distinta permaneceu sem resumo; callbacks sem token foram recusados; o payload não expôs o segredo de callback; token usage continuou sendo contabilizado pelos cenários da Responses API. No MySQL, a migração executou duas vezes, preservou a linha anterior e armazenou/recuperou resumo UTF-8 de 80.000 caracteres com uma única coluna.
- Ambiente: engine e plugins Docker conferidos com `docker version`, `docker buildx version` e `docker compose version`; utilizado exclusivamente o projeto de homologação `aihub-ed5fa829-9717-4548-80fc-d100a0fd9341-27ed6b9a4d`, sem portas públicas, host network, privilégios ou bind mounts. Limpeza solicitada com `docker compose -p aihub-ed5fa829-9717-4548-80fc-d100a0fd9341-27ed6b9a4d -f .local-reasoning-validation/compose.yml down --volumes --remove-orphans`.
...
### 2026-09-13 — Correção das falhas de CI após integrar os resumos

- Solicitação: acessar o GitHub Actions com erro e realizar os ajustes locais necessários.
- Execuções consultadas: CI da `main` [34780893629](https://github.com/paulofor/ai-hub/actions/runs/34780893629), commit `a6f431a`, e CI do PR #704 [34780889211](https://github.com/paulofor/ai-hub/actions/runs/34780889211). Backend e sandbox falharam; frontend passou; imagens e deploy foram bloqueados pelas dependências do workflow.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. Na árvore integrada, a Responses API executa o coletor por identificador e também a concatenação antiga, duplicando o último resumo; uma fixture anterior do App Server emite apenas `delta`, sem `itemId`/`summaryIndex` exigidos pelo protocolo; e o teste de callback cria registros numa transação ainda não confirmada, invisível à transação `REQUIRES_NEW` usada pela sincronização. A validação anterior descrita neste diário usou uma base anterior à `main` integrada e um subconjunto de testes, não a árvore final que o CI executou.
- Matriz definida antes da reprodução/validação: reproduzir as falhas dos dois jobs; executar a suíte completa do orquestrador (incluindo resumo, eventos inválidos, retomadas, isolamento e métricas), a suíte completa do backend e os checks de migrations do CI; encadear payload sintético do App Server → callback HTTP autenticado → persistência H2 → detalhe → Chromium desktop/mobile; validar lint/build do frontend e os demais checks locais do CI (MCP e ciclo Docker protegido). Manter credenciais externas fora dos processos de teste, banco H2 exclusivo e dados sintéticos; nenhuma chamada real de modelo nem alteração de produção.
- Critério: após a última correção, duas rodadas locais completas e consecutivas sem falhas dessa matriz; revisar o diff e registrar resultados e limites. Docker/Buildx/Compose foram conferidos; qualquer topologia temporária usará exclusivamente `aihub-09951f3b-f75e-4dff-8336-cced55843cf0-23171a8b23` e será removida ao terminar. Não há alteração de changelog nem necessidade de workflow remoto para testar esta correção.
- Reprodução local confirmada antes da correção: 4 falhas em 113 testes do orquestrador e 1 falha no teste de callback, com as mesmas asserções do Actions. O log do backend confirmou que o registro não era visível na nova transação. Após ajustar somente o teste, o callback já persistiu o resumo sem alterar a transação do serviço.
- Correções: removida a concatenação antiga, mantendo o coletor único por resposta/item; completada a fixture com `threadId`, `turnId`, `itemId` e `summaryIndex`; removida a transação abrangente do teste do backend e adicionada limpeza dos IDs sintéticos em `@AfterEach`. Cada leitura posterior usa uma nova transação do repositório, validando o dado realmente persistido. O protocolo foi conferido no schema versionado e na [documentação oficial do App Server](https://learn.chatgpt.com/docs/app-server#item-deltas), usando OpenAI Docs.
- Incidente da investigação: a primeira consulta ao remoto Git exibiu uma credencial embutida na URL na saída da ferramenta. O valor não foi copiado para arquivos nem reproduzido nas mensagens; o usuário foi avisado e orientado a revogá-lo/substituí-lo. Os processos de teste usam uma lista restrita de variáveis de ambiente, sem as credenciais de APIs do ambiente.
- Homologação concluída após a última correção: duas rodadas completas e consecutivas, cada uma com **113 testes do orquestrador, 132 do backend, 9 do MCP e 4 E2E do frontend** (258 por rodada), sem falhas ou testes ignorados. Também passaram, em ambas, build TypeScript do orquestrador, lint/build do frontend, unicidade das versões Flyway, migration de recuperação em MySQL 5.7/PostgreSQL 16 reais, contrato e E2E da borda pública MCP, ciclo protegido de Docker com transporte SSH simulado localmente e `git diff --check`.
- Evidência do fluxo de resumos: o App Server simulado por processo/JSON-RPC gerou o payload com `Resumo público validado por JSON-RPC.`; o teste do backend aplicou esse mesmo payload pelo callback autenticado, persistiu em H2 e exportou o detalhe; os testes de navegador consumiram esse detalhe no Chromium desktop e na emulação de iPhone 15 Pro. Texto e resposta final permaneceram separados e iguais entre módulos nas duas rodadas. Ausência de resumo, polling, formato legado, autenticação, segregação entre solicitações e métricas estão cobertos pelos testes. Screenshots foram inspecionadas localmente e permanecem fora do Git.
- Execução reproduzível nesta sandbox: `python3 .local-ci-validation/round.py 1` e `python3 .local-ci-validation/round.py 2`; o runner encadeia os comandos versionados do CI e os testes E2E já existentes, com `REASONING_SUMMARY_E2E_PAYLOAD`/`REASONING_SUMMARY_E2E_DETAIL`. Logs por etapa em `.local-ci-validation/round-{1,2}-*.log`; payloads, detalhes e capturas em `.local-ci-validation/round-{1,2}/`, excluídos localmente do Git.
- Limites: provedores de modelo foram substituídos por doubles locais; não houve geração paga nem teste em produção. O frontend mantém dois avisos preexistentes fora deste ajuste (dependência `selectedProcessId` em hook e tamanho do bundle), sem erros de lint/build. Nenhum workflow ou arquivo de migration foi alterado.
- Encerramento: executado `docker compose -p aihub-09951f3b-f75e-4dff-8336-cced55843cf0-23171a8b23 -f <compose-da-suite> down --volumes --remove-orphans` para as três topologias; confirmados zero containers, volumes e redes do projeto e zero containers das fixtures remotas. Diff funcional revisado; apenas três arquivos de código/teste e este diário alterados. Nenhum commit de entrega, push, PR, execução de pipeline, SSH operacional ou deploy foi realizado.

### 2026-09-14 — Gráficos de tokens no dashboard

- Solicitação recebida: acrescentar aos painéis diário e semanal do dashboard um gráfico de tokens com os mesmos critérios dos gráficos de tempo, solicitações e interações.
- Pergunta explícita de causa raiz: “por que os tokens não apareciam nesses gráficos?”. Resposta: o contrato de métricas agregava e devolvia apenas quantidade de solicitações, interações e duração; embora `total_tokens` já fosse persistido por solicitação, ele não era selecionado pelas consultas nem acumulado nos buckets temporais, portanto o frontend não possuía dados de tokens para representar.
...
## 2026-09-15 15:24:35 UTC-3

- Solicitação: investigar as últimas solicitações e resolver a suspeita de perda de código. Histórico Git local aprofundado em 120 commits, sem alterar a branch; entregas recentes do AI Hub estão presentes. Consultas de leitura das solicitações #2837–#2845 identificaram o incidente no Marketing Hub: a #2838 relata a publicação das telas de tipos de produto, enquanto #2844/#2845 apontam a revisão `12e2e35ab4db307990924062eba6e243cb054062` ausente do GitHub e a retomada protegida aguardando integração.
- Pergunta explícita: “por que esse erro aconteceu?”. Evidência inicial: o GitHub não encontra a revisão; os diffs acessíveis das respostas posteriores não contêm as telas. A fonte original continua sendo procurada nos registros e diretórios de build autorizados, apenas por leitura. No orquestrador versionado, foi identificado um risco concreto a reproduzir: o encerramento usa `git checkout -B` sobre a branch remota depois dos testes, podendo abandonar commits locais, e falhas de publicação são apenas registradas antes da remoção incondicional do workspace. Isso ainda não comprova qual operação ocorreu historicamente na #2838.
- Alternativas de preservação avaliadas: (1) reaplicar o patch sobre a branch remota, baixo esforço mas perde identidade dos commits e pode mudar a árvore já testada; (2) integrar/rebasear automaticamente ao publicar, esforço maior e exige repetir a validação de uma árvore possivelmente diferente; (3) preservar o HEAD validado, aceitar somente publicação fast-forward e conservar workspace/patch diante de divergência ou falha, esforço moderado e preservação de código e identidade. Escolhida a terceira, condicionada à reprodução local.
- Matriz definida antes dos testes: Git real em repositórios locais descartáveis; branch nova/existente; commits produzidos durante a tarefa; alterações pendentes; avanço concorrente do remoto; indisponibilidade do remoto e falha de push; conservação de código/patch e sinalização de falhas; ausência de PR quando desabilitado; regressão da suíte completa do orquestrador. Sem alteração de UI, navegadores não se aplicam a essa correção de transporte Git. Provedores e GitHub serão doubles, sem tokens reais, gastos ou publicação. Após defeito corrigido, duas rodadas locais completas consecutivas sem falhas e revisão do diff.
- Limitação observada: o helper SSH recusou a autenticação em `root@163.245.203.201`; nenhuma proteção foi desativada nem outra credencial tentada. A consulta por SSH ao host administrativo do Marketing Hub está disponível e é exclusivamente de leitura. Fontes auxiliares ficam em `.local-code-recovery/`, excluído localmente do Git.

## 2026-09-15 15:37:26 UTC-3

- Recuperação efetiva: as credenciais de banco já exportadas permitiram consultar, somente por leitura, `responses` no intervalo de término da #2838. A resposta **5084**, de `2026-09-15T08:23:49.628Z`, conserva o diff original. O acesso SSH anteriormente solicitado deixou de ser necessário. O registro estava fora das dez respostas expostas pela API de projetos, explicando por que a listagem recente não o mostrava.
- Os **26 arquivos** das telas de tipos de produto foram restaurados em um checkout local da `main` atual do Marketing Hub, sem conflitos. Apenas cinco arquivos de pesquisas já integrados foram excluídos. Pacote durável, manifesto e evidências preparados em `docs/recuperacao/solicitacao-2838/`; o diretório temporário de validação não é a única cópia recuperada. A fonte, os testes e o documento original de homologação foram preservados.
- Reprodução e correção do AI Hub: após corrigir a configuração da própria fixture para fornecer `githubToken` sintético, seis dos sete testes falhavam no código original. Confirmados abandono de commits ao reconfigurar a branch sobre o remoto, sucesso indevido após falha de publicação e remoção do único workspace com código não publicado. O encerramento agora preserva o HEAD validado, exige ancestralidade antes do push, diferencia ausência de branch de falha de consulta, propaga falhas e retém o workspace em caso de código não publicado ou erro. Mensagens de falha de push não reproduzem stderr com credenciais. Teste inclui trabalho em branch local diferente, alterações pendentes, concorrência e falha de validação.
- Homologação do orquestrador: duas rodadas finais completas consecutivas, **120/120 testes aprovados em cada uma**, sem falhas ou ignorados. Sete cenários novos executam Git real com remotos locais descartáveis e GitHub/modelo simulados. Não houve commit/push no repositório de entrega.
- Homologação da fonte recuperada: matriz original executada uma vez integralmente, **177 testes de backend + 8 com MySQL 5.7 isolado + 43 de frontend**, todos aprovados; **12 grupos de navegação** em Chromium desktop/iPhone 15 Pro/Pixel 7, 42 consultas locais e zero mutações, chamadas pagas ou erro JS. TypeScript, build, Spotless, Prettier e diff passaram. A primeira rodada não revelou defeito na fonte recuperada, portanto não foi repetida apenas por contagem mínima. Capturas desktop/mobile foram inspecionadas.
- Limite explícito: recuperado o conteúdo-fonte, não o objeto Git do SHA antigo `12e2e35ab4db307990924062eba6e243cb054062`. O coordenador produtivo continua em `AWAITING_MERGE`; não foi alterado, liberado nem apontado arbitrariamente para outra revisão. O pacote deve ser integrado ao Marketing Hub com a comprovação da revisão recuperada pelo fluxo de PR do usuário. A prevenção pertence ao AI Hub. Não foram abertos PRs, disparados workflows ou realizados deploys.
- Limpeza: Compose exclusivo `aihub-072c319f-d78f-4fc5-8f6b-cf6a361bc695-211b9cd21f` removido com `down --volumes --remove-orphans`, confirmando zero containers, redes e volumes desse projeto. Workspaces sintéticos retidos pelos testes dentro do repositório também foram removidos.
- Incidente de ferramenta: uma consulta inicial ao remoto Git exibiu a credencial embutida na URL. O valor não foi copiado para arquivos de entrega nem repetido nas mensagens; o usuário foi avisado e orientado a substituir a credencial. As consultas posteriores foram sanitizadas e os testes receberam lista restrita de variáveis, sem credenciais reais de APIs/banco.

## 2026-09-15 15:46:04 UTC-3

- Na conferência da continuidade da recuperação, investigada explicitamente a pergunta “por que a publicação ainda poderia ficar bloqueada depois de recuperar e integrar os arquivos?”. O coordenador já permite `resume` auditado com outra revisão integrada e nova evidência, mas o reconciliador ignorava `integrated_commit` e continuava comparando a preparação antiga, inclusive depois de uma retomada parcial. Três testes locais reproduziram a falha: ausência de dispatch após retomada válida, retomada parcial voltando a aguardar o SHA desaparecido e aceitação de registro incompleto por outra condição do fluxo.
- Alternativas: reconstruir o objeto Git original preservaria o SHA, mas ele não foi localizado; substituir diretamente a preparação antiga eliminaria a rastreabilidade; usar a revisão já comprovada e auditada pelo método `resume` preserva o histórico, requer baixo esforço e mantém as verificações de integração. Escolhida a terceira. Não existe mudança automática de revisão enquanto o registro estiver em `AWAITING_MERGE`.
- Correção complementar aplicada somente no checkout local do Marketing Hub: o reconciliador considera a revisão/evidência registradas por `resume`, conserva `automatic_resume` e reconsulta a integração da revisão inicial e da autorizada. Registro incompleto continua bloqueado. O cânone foi atualizado e três testes de regressão foram adicionados.
- Matriz final ampliada antes da rodada: suíte completa de 120 testes do AI Hub, 33 testes do coordenador, 44 do reconciliador e matriz completa de recuperação das telas (177 backend, 8 MySQL, 43 frontend, 12 grupos de navegador e verificações de qualidade/segregação/limpeza). Duas rodadas completas consecutivas após esta última correção. Nenhuma mutação produtiva, PR ou publicação foi executada.

## 2026-09-15 15:58:15 UTC-3

- Homologação final concluída: `round-2` e `round-3` passaram integralmente e consecutivamente após a última correção funcional, com **425 testes e 12 grupos de navegação em cada rodada**, sem falhas ou testes ignorados. Cada rodada incluiu 120 testes do AI Hub, 33 do coordenador, 44 do reconciliador, 177 do backend de tipos/produtos/arquitetura, 8 com MySQL 5.7 real isolado e 43 do frontend. TypeScript, build, Spotless, Prettier e diff aprovados nas duas execuções.
- Evidência ponta a ponta: navegação de Opala, Quartzo e Safira, retorno/refresh/links, teclado, falhas 409/404/ID inválido/503 e nova tentativa em Chromium desktop, iPhone 15 Pro e Pixel 7 emulados; 42 consultas locais e zero mutações por rodada. Capturas inspecionadas. Modelo, GitHub e transporte de publicação foram doubles locais; dados sintéticos segregados e nenhuma chamada paga ou escrita produtiva. Limites: emulação não substitui aparelhos físicos; fixture não fornece o logotipo global preexistente, e o build mantém seu aviso preexistente de bundle grande.
- Pacote final em `docs/recuperacao/solicitacao-2838/`: **26 arquivos originais recuperados e cinco complementares** de reconciliador/testes/documentação. SHA-256 do patch: `5f1654b055b4af1cd6376e1a3330d507725920d4e21ea24da5470e7f13d83349`. Conferidos aplicação sobre índice temporário da main limpa do Marketing Hub, igualdade byte a byte dos 31 arquivos com o checkout recuperado, aplicação reversa e ausência de erros de whitespace. Manifesto separa fonte recuperada e complementos; `validacao.json` registra contagens, resultados, hashes de fontes/logs e limites.
- Limpeza final: `docker compose -p aihub-072c319f-d78f-4fc5-8f6b-cf6a361bc695-211b9cd21f ... down --volumes --remove-orphans`; confirmados zero containers, redes e volumes desse projeto. Removidos oito workspaces sintéticos retidos pelas suítes antigas, preservando o checkout e as evidências reais da recuperação.
- Entrega pronta localmente nos dois repositórios, com pacote textual do Marketing Hub preservado no AI Hub. Nenhum commit de entrega, push, PR, pipeline ou deploy executado. O objeto Git antigo continua indisponível; a fonte e a correção da retomada estão recuperadas e homologadas. O coordenador produtivo não foi alterado: a continuidade exige integração pelo fluxo de PR do usuário e retomada auditada com a revisão efetivamente integrada e sua nova evidência, mantendo a preparação histórica intacta.

## 2026-09-15 20:01:10 UTC-3

- Solicitação: remover a orientação de executar duas rodadas de testes e tornar a validação mais simples e econômica.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. Resposta: a exigência estava escrita diretamente na instrução operacional enviada pelo orquestrador a todos os perfis e protegida por asserções literais em dois testes; por isso os agentes repetiam a matriz completa após qualquer correção, mesmo quando uma validação direcionada seria suficiente.
- Correção na causa: a instrução agora pede uma rodada dos testes relevantes e, se houver correção, somente as validações necessárias para confirmá-la e cobrir regressões relacionadas. A repetição integral por contagem mínima foi proibida.
- Proteção contra regressão: os testes dos prompts MKT e padrão passaram a exigir a orientação econômica e a confirmar que a expressão `duas rodadas` não é mais enviada ao modelo.

## 2026-09-16 — Viabilidade de medir percentual de cota por solicitação

- Consulta sobre capturar o saldo percentual mostrado no Codex antes/depois e persistir a diferença. Analisados o anexo do usuário (94% semanais restantes), `apps/sandbox-orchestrator/src/jobProcessor.ts:1397` (evento de tokens já tratado) e `exemplos/codex-rs/app-server/README.md:993-1002` (consulta e campos de cota). Confirmada a interface na documentação oficial https://learn.chatgpt.com/docs/app-server.
- Alternativas comparadas: (1) dois snapshots via `account/rateLimits/read`, esforço baixo e boa aderência ao percentual solicitado, mas sujeitos ao consumo concorrente e à atualização do provedor; (2) snapshots com eventos `account/rateLimits/updated` e tokens por execução, esforço moderado, melhor auditabilidade e mesma limitação de atribuição da cota compartilhada; (3) serialização das tarefas da conta, esforço maior e perda de vazão, sem impedir uso externo. Recomendada a segunda, sem implementar nesta consulta de viabilidade.
- Persistência proposta: solicitação/execução, identidade interna da conta, identificador do limite, janela e reset, instantes e valores inicial/final, diferença em pontos percentuais, tokens e qualidade da medição. Comparar somente a mesma conta/limite/janela; reset ou leitura ausente torna o delta indisponível. Concorrência torna a atribuição estimada. Delta zero não comprova consumo zero; percentual não equivale a custo monetário.
- Validação documental e inspeção local; sem chamada autenticada de cota, alteração funcional, migração, teste de execução, PR ou publicação. Única alteração: este registro obrigatório.

## 2026-09-16 — Implementação do consumo de cota nas solicitações

- Solicitação: implementar a coleta inicial/final da cota, persistir a diferença e exibi-la nas últimas execuções e no detalhe. Investigação orientada pela pergunta explícita “por que esse dado ainda não aparece por solicitação?”. Causa: o orquestrador só coletava tokens; não havia snapshots de cota, persistência ou contrato de exibição.
- Alternativas comparadas: duas leituras (baixo esforço, pouca auditoria); leituras com eventos e tokens (esforço moderado, melhor rastreabilidade); serialização da conta (alto custo de vazão, sem eliminar uso externo). Escolhida a segunda. Na persistência, comparados campos fixos, tabela própria e JSON versionado; escolhido JSON nullable para preservar todas as janelas sem ampliar o escopo de entidades/joins. Justificativas e matriz em `docs/homologacao/consumo-cota-por-solicitacao.md`.
- Implementação local: snapshots com conta pseudonimizada, janela/reset/instantes, delta em p.p. por janela, eventos e concorrência, timeout de telemetria e fechamento em sucesso/falha/interrupção; indisponibilidade explícita para dados incomparáveis. Migração V52 para H2/MySQL/PostgreSQL. Callback/polling persistem a medição sem apagar o final por baseline atrasado. Histórico paginado e detalhe exibem consumo estimado nos três perfis ChatGPT, com inicial/final e horários no detalhe.
- Homologação: 148 testes do orquestrador e 63 testes Java passaram; dois casos adicionais (timeout de telemetria e janelas independentes) validados na rodada direcionada de 39 testes. Payload real produzido pelo double de execução alimentou callback Java, persistência H2 e detalhe exportado aos testes de navegador. Oito cenários de cota em desktop/iPhone 15 Pro emulado e quatro regressões de resumo passaram. Migração MySQL 5.7 executada localmente duas vezes, preservando dados legados e persistindo delta 2. Builds e revisão de diff concluídos.
- Ajustes iterativos: mock de navegador interceptava também `/src/api/client.ts`; corrigido na fixture. Inspeção visual identificou formatação da cota em fuso distinto; reutilizada a função de datas da aplicação. Reexecutados apenas os cenários afetados. Sem repetição integral por contagem mínima.
- Limites: precisão/atraso do provedor não medidos com conta real; consumo compartilhado permanece estimado, 0 p.p. não prova ausência de consumo. Registros antigos/perfis sem cota ChatGPT ficam indisponíveis. PostgreSQL validado no H2 por compatibilidade SQL; mobile emulado não substitui Safari/aparelho físico. Aviso preexistente de bundle grande no frontend.
- Segregação: dados sintéticos, sem credenciais reais, APIs pagas ou escritas produtivas. Compose exclusivo `aihub-3290a142-4996-42f3-9bfd-e264fcedb3e6-3a567643fe` removido com volumes/órfãos. Nenhum commit, push, PR, pipeline ou deploy realizado; mudanças prontas localmente para o fluxo de publicação do usuário.
## 2026-09-16 — Correção do workflow após a migração V52

- Solicitação: corrigir https://github.com/paulofor/ai-hub/actions/runs/35041834616/job/104623170890. Logs consultados via GitHub CLI; HEAD local corresponde ao commit do run `e007b75801e74b8ab87445e9b229cbe3e47fc165`. Backend falhou em dois dos 137 testes; frontend e sandbox passaram.
- Pergunta explícita: “por que esse erro aconteceu?”. Os testes `CodexRequestMaxWaitMigrationTest` e `PublicProxyRecoveryMigrationTest` executavam toda a cadeia H2, mas exigiam versão final literal `51`. Com V52, a migração concluía corretamente e essa expectativa obsoleta falhava. Reprodução local antes da edição: dois testes, duas falhas `expected: 51, but was: 52`. A validação anterior da funcionalidade de cota havia executado apenas um subconjunto da suíte Java, deixando esses testes de fora.
- Alternativas: (1) atualizar o literal para 52, esforço mínimo, mas recorrência na próxima migração; (2) fixar o target nas migrações específicas, esforço baixo e isolamento, mas menor cobertura da cadeia atual; (3) verificar ausência de migrações pendentes e validação Flyway bem-sucedida, esforço baixo e cobertura da cadeia completa sem acoplamento ao número final. Escolhida a terceira por corrigir a causa da fragilidade. Preservadas as verificações de colunas existentes e unicidade de request_id; nenhum SQL ou código produtivo foi alterado.
- Validação local do mesmo passo do workflow: `validate-migration-versions.sh`, `test-public-proxy-recovery-migrations.sh` em MySQL 5.7/PostgreSQL reais isolados e `mvn -f apps/backend -B test`; 137 testes aprovados, zero falhas, erros ou ignorados. Diff revisado e `git diff --check` aprovado. Sem mudança de interface, não foi necessário repetir testes de navegador.
- Docker, Buildx e Compose verificados antes do uso. Projeto exclusivo `aihub-0082eb36-4967-46bb-8f76-3636b47421dd-8875ff6e7e`, dados sintéticos; limpeza por `down --volumes --remove-orphans` no trap do script e ausência de containers confirmada. Nenhum commit, push, PR, pipeline ou deploy realizado; correção pronta no workspace.

## 2026-09-16 — Recuperação de conexão durante migração no deploy

- Solicitação: corrigir a falha da GitHub Action de deploy em que o backend encerrava durante o Flyway com `Unable to restore connection to its original state` / `Connection is closed`.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. A migração MySQL V52 executa DDL, que possui commit implícito; a conexão foi encerrada depois da execução e antes de o Flyway registrar/restaurar seu estado. Embora existisse uma segunda configuração com retentativa, ela estava condicionada à existência antecipada do bean `Flyway` e não foi a estratégia chamada pelo stack trace (`FlywayConfig:18`). A estratégia efetiva fazia somente `repair()` e uma única chamada de `migrate()`, portanto a falha transitória derrubava o backend e, por consequência, a verificação do deploy.
- Correção na causa: as duas estratégias concorrentes foram consolidadas em um único bean efetivo. Ele executa `repair()` antes de cada tentativa, repete `migrate()` com uma nova aquisição de conexão e aplica backoff configurável de 5 até 60 segundos. Isso também recupera o caso MySQL em que o DDL já foi confirmado, mas a linha de histórico ficou marcada como falha; a migração V52 já é idempotente e preserva a coluna existente.
- Proteção contra regressão: criado teste unitário que simula exatamente `Connection is closed` na primeira migração e comprova a sequência `repair/migrate` em duas tentativas, além do cenário com reparo automático desativado.
- Validação: teste direcionado aprovado (2 testes), suíte completa do backend aprovada (139 testes), versões Flyway únicas nos três bancos e `git diff --check` sem erros.

## 2026-09-16 — Resumo semanal da cota no histórico

- Solicitação: manter no histórico das execuções somente a cota semanal principal do Codex marcada na tela, sem poluir o cartão com as janelas de cinco horas e de inferência do modelo base.
- Pergunta explícita de causa raiz: “por que esse erro aconteceu?”. O mesmo componente reutilizava, sem uma política própria de resumo, todas as janelas devolvidas pelo provedor tanto no cartão compacto do histórico quanto na página de detalhes. Assim, a inclusão correta das novas métricas fez o histórico apresentar informação excessiva.
...
- Investigação orientada pela pergunta explícita “por que esse erro aconteceu?”. Causa raiz: `SalesImpactTimeline` em `apps/frontend/src/pages/DashboardPage.tsx` agrupava as avaliações por dia e renderizava somente marcadores HTML posicionados; não havia caminho SVG, segmento CSS ou outro elemento responsável por conectar a série.
- Alternativas comparadas: (1) segmentos CSS entre pontos, menor esforço, mas frágil em redimensionamento e manutenção; (2) biblioteca externa de gráficos, mais recursos, porém nova dependência e custo visual desnecessário; (3) SVG nativo responsivo para a linha, esforço baixo/médio, sem dependência e preciso no mesmo espaço do componente. Escolhida a alternativa 3 por corrigir a causa no frontend existente e preservar contrato da API, escala, filtros e alternância do período.
- Implementação local: calculadas coordenadas X/Y da série de médias na escala 1–5; adicionada linha SVG verde com `path` responsivo, mantendo marcadores circulares e rótulos em HTML para evitar distorção quando a largura do gráfico muda. Atualizados os nomes acessíveis para identificar o gráfico de linha e o E2E passou a exigir um caminho com segmento.
- Ajuste iterativo: a primeira versão colocou círculos dentro de um SVG com `preserveAspectRatio="none"`, o que os deformou em elipses; a inspeção visual no Chromium identificou a causa e a versão final deixou somente a linha no SVG, sobrepondo pontos HTML circulares, com margem vertical para notas 1 e 5 não serem cortadas.
- Validação: `npm ci --no-audit --no-fund`; `npm run build` aprovado; `npm run lint` sem erros (um warning preexistente de dependência de hook em `CodexChatgptPage.tsx`); cenário direcionado `renders the dashboard shell` aprovado duas vezes; cenário direcionado de tokens aprovado isoladamente; captura `/tmp/ai-hub-dashboard-media-diaria-21-dias.png` inspecionada e confirmou linha, pontos e rótulos.
- Suíte ampla: a rodada `npm run test:e2e` terminou com 35/40 aprovados. Cinco falhas ficaram fora do escopo do gráfico: chamadas não interceptadas ao backend local (`ECONNREFUSED 127.0.0.1:8081`), expectativa de conversa sem resposta sintética, alerta de interação, seletor estrito de cartão de código e placar no fluxo MKT; uma rodada paralela também atingiu limite de threads do Chromium. O teste relevante do dashboard foi reexecutado com um worker e passou; nenhum desses defeitos foi atribuído à alteração por não haver relação causal com `DashboardPage`.
- Segregação e entrega: somente dados sintéticos das fixtures, sem credenciais reais, APIs pagas ou escrita produtiva. Nenhum commit, push, PR, pipeline, deploy ou publicação realizado; mudanças ficam prontas na worktree para o fluxo de PR solicitado pelo usuário.

## 2026-09-17 — Registro dos itens de prompt do tipo Tela por solicitação

- Solicitação: registrar quais itens de prompt do tipo `Tela` foram usados ao enviar uma solicitação e exibi-los tanto no detalhe geral quanto no detalhe de solicitações com nota 5 em vendas.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** Os itens `Tela` eram tratados somente como conveniência transitória do formulário: marcar o checkbox copiava a frase para o campo de texto, mas o `POST /codex/requests` enviava apenas o prompt final. Como identidade, nome e conteúdo do item não atravessavam o contrato da API, não havia informação persistida que as telas de detalhe pudessem recuperar com segurança.
- Correção na causa: o envio do perfil ChatGPT agora inclui um snapshot dos itens `Tela` selecionados (id, nome e frase); o backend normaliza e persiste esse snapshot em uma coluna JSON textual própria. O snapshot preserva o que foi usado mesmo se o cadastro do item for renomeado ou alterado posteriormente. Migrações V53 idempotentes foram adicionadas para H2, PostgreSQL e MySQL.
- Apresentação: criado um bloco compartilhado `Itens de tela usados`, exibido somente quando houver itens registrados, no detalhe geral da solicitação e no detalhe `Nota 5 em Vendas`. Solicitações antigas permanecem compatíveis e simplesmente não exibem o bloco.
- Proteção contra regressão: as fixtures E2E dos dois detalhes passaram a fornecer um item de tela e verificar título e nome renderizados. O parser do frontend valida o formato retornado antes de expô-lo às páginas.

## 2026-09-17 — Consumo diário da cota semanal no quadro operacional

- Solicitação: substituir no quadro flutuante do ChatGPT Marketing a contagem de interações pela soma, no dia operacional, dos pontos percentuais consumidos da cota semanal da conta, reaproveitando a medição já registrada em cada solicitação.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** O quadro consumia somente os agregados históricos antigos (`requestCount`, `interactionCount`, duração e tokens). Embora cada solicitação já persistisse `quotaUsage`, a API de métricas não lia esse campo; portanto, o frontend não tinha um total diário correto e não poderia obtê-lo da lista paginada de solicitações sem subcontagem.
- Correção na causa: o backend passou a consultar os snapshots de cota do mesmo recorte temporal/perfil, selecionar apenas a janela primária Codex de 10.080 minutos (semanal) e somar `consumedPercentagePoints`. Snapshots ausentes, inválidos e janelas de 5 horas são ignorados; sem nenhuma medição válida, a API retorna ausência em vez de inventar zero.
- Apresentação: o cartão antes rotulado `Interações` agora mostra `Cota semanal` e o consumo do dia em `p.p.`, com até quatro casas decimais e formatação pt-BR. O alerta visual de inatividade continua baseado nos tokens da solicitação em execução e recebeu cobertura E2E coerente com esse comportamento.
- Proteção contra regressão: teste unitário do serviço cobre soma da janela semanal, descarte da janela de 5 horas e tolerância a JSON histórico inválido. As fixtures E2E validam o valor `1,25 p.p.` no quadro e a convivência com o alerta de inatividade. O teste visual direcionado também gera screenshots do quadro operacional.

## 2026-09-17 — Modelo e raciocínio no diálogo das solicitações

- Solicitação: exibir, no cabeçalho de cada resposta do modelo dentro do diálogo, o modelo efetivamente usado e o tipo de raciocínio da execução.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** O diálogo associava a resposta à solicitação somente para recuperar ambiente e status; embora `CodexRequest` já trouxesse `model` e `reasoningEffort`, esses campos não eram projetados no `ChatMessage` nem renderizados no cartão. Assim, a informação existia no histórico, mas era descartada na representação persistida do diálogo.
- Correção na causa: `ChatMessage` passou a preservar modelo e esforço de raciocínio na criação, atualização e restauração via `localStorage`; para conversas antigas, a renderização também recupera os dados da solicitação carregada pelo `requestId`. O cabeçalho da resposta agora mostra `Modelo usado` e `Tipo de raciocínio`, com os níveis traduzidos para português.
- Proteção contra regressão: o cenário E2E de diálogo de Marketing recebeu metadados persistidos e asserções para `gpt-5.6-sol` e `Extra alto`, além de captura visual da posição dos campos.

## 2026-09-17 — ShellCheck disponível na sandbox

- Solicitação: disponibilizar ShellCheck na sandbox para complementar `bash -n` com análise estática dos scripts.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** A imagem `apps/sandbox-orchestrator` não instalava o pacote `shellcheck`, embora `apps/sandbox-orchestrator/tests/test-ssh-docker-full.sh` já tentasse executá-lo; a sandbox atual também não tinha o comando. Por isso, a checagem estática falhava por dependência ausente e a validação ficava restrita ao `bash -n`.
- Alternativas comparadas: (1) instalar manualmente somente na sessão atual, baixo esforço mas não reproduzível; (2) instalar `shellcheck` na imagem versionada, validar no build, expor no preflight e documentar o uso; (3) instalar somente no CI, cobrindo PRs mas não a investigação interativa na sandbox. Escolhida a alternativa 2, complementada pelo CI, por corrigir a causa na origem e manter o ambiente reproduzível.
- Correção aplicada: `apps/sandbox-orchestrator/Dockerfile` instala `shellcheck` pelo pacote Debian e valida `shellcheck --version`; o preflight do `jobProcessor` reporta a ferramenta e instrui a sequência `bash -n` + `shellcheck`; README e arquitetura documentam o contrato.
- Validação versionada: criado `.github/scripts/validate-shell-scripts.sh`, que encontra scripts shell rastreados, exclui vendor/terceiros e executa `bash -n` e `shellcheck -x`. O job `sandbox` do CI e o ensaio completo Docker/SSH instalam/disponibilizam ShellCheck e executam esse validador antes dos testes.
- Ajuste iterativo: a primeira análise encontrou cinco avisos reais em `infra/setup_vps.sh` (`SC2155`, `SC1091` e `SC2028`); as declarações/atribuições foram separadas e o `echo` foi trocado por `printf`, deixando os 19 scripts de primeira parte limpos.
- Homologação local: `shellcheck --version` retornou 0.9.0; `bash .github/scripts/validate-shell-scripts.sh` passou; `actionlint -shellcheck= .github/workflows/ci.yml` passou; `npm --prefix apps/sandbox-orchestrator test` passou com 150/150; `git diff --check` passou; Docker Engine, Buildx e Compose v2 foram verificados; o build da imagem passou e `docker run` confirmou `/usr/bin/shellcheck` versão 0.9.0. A execução por `docker compose run` ficou limitada porque a rede externa preexistente `public-net` não existe na sandbox; a imagem foi verificada diretamente e a limpeza Compose foi executada. Nenhum commit, push, PR ou deploy foi realizado.

## 2026-09-17 — Navegação para a próxima solicitação no detalhe Codex

- Solicitação: colocar um botão `Próximo` ao lado de `Anterior` no detalhe de uma solicitação Codex.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** A navegação existente foi implementada de forma unilateral: backend e frontend só possuíam contrato, consulta e estado para localizar o ID imediatamente inferior (`previous`). Não havia endpoint nem consulta ao ID imediatamente superior, portanto adicionar apenas um botão visual produziria uma ação sem fonte confiável de destino.
...
## 2026-09-18 — Análise de gráficos de cota semanal no dashboard

- Solicitação: exibir no dashboard gráficos diário e semanal de consumo de cota semanal em pontos percentuais (p.p.), com marca visual de meta diária em 14 p.p.
- Pergunta explícita de causa raiz: **“por que esse dado não aparece no dashboard?”** Embora o backend já calcule `weeklyQuotaConsumedPercentagePoints` para a janela agregada do dia, os métodos que montam `series.daily` e `series.weekly` preenchem esse campo com `null`. Consequentemente, a tela `DashboardPage` só consegue desenhar Tempo, Solicitações, Interações e Tokens; não há série histórica de p.p. para o gráfico solicitado.
- Evidências examinadas: `apps/frontend/src/pages/DashboardPage.tsx` consome `series.daily`/`series.weekly` e o componente reutilizável `MiniBarChart`; `apps/backend/src/main/java/com/aihub/hub/service/CodexRequestService.java` calcula a cota somente em `buildMetricWindow`, enquanto `toDailyMetricWindows` e `toMetricWindows` retornam cota nula; `apps/backend/src/main/java/com/aihub/hub/dto/CodexDashboardMetrics.java` já prevê o campo necessário em cada bucket.
- Alternativas comparadas: (1) gráfico calculado exclusivamente no frontend a partir do agregado atual — esforço baixo, mas impossível obter histórico fiel; (2) acrescentar uma série paralela específica de cota — preserva as métricas atuais, porém duplica contrato e lógica de período; (3) preencher `weeklyQuotaConsumedPercentagePoints` nos buckets diário e semanal existentes e renderizar dois `MiniBarChart` dedicados, com linha de referência diária em 14 p.p. Escolhida a alternativa 3 por reutilizar o contrato e o padrão visual existentes, manter uma única fonte de verdade e tornar os dados auditáveis por período.
- Orientação de produto: o gráfico diário deve usar o mesmo dia operacional já aplicado ao dashboard (02h–02h), barras em p.p., tooltip com data/valor/meta e linha horizontal rotulada “Meta diária: 14 p.p.”; o gráfico semanal deve consolidar os p.p. por semana operacional e não repetir a linha diária, evitando sugerir que 14 é uma meta semanal. Estados sem snapshot válido devem aparecer como indisponíveis, nunca como zero.
- Nenhum código funcional, commit, Pull Request, publicação ou deploy foi realizado nesta análise.

## 2026-09-18 — Gráficos diário e semanal de cota semanal

- Solicitação implementada: exibir no dashboard o consumo de cota semanal em p.p. nos recortes dos últimos 14 dias e das últimas 10 semanas, com marca de meta diária em 14 p.p. exclusivamente no gráfico diário.
- Pergunta explícita de causa raiz: **“por que os gráficos não poderiam ser implementados somente na tela?”** As séries do endpoint de métricas eram montadas com `weeklyQuotaConsumedPercentagePoints` nulo, embora o agregado do dia já soubesse calcular a cota; o frontend não possui snapshots por solicitação e não conseguiria reconstruir histórico confiável por conta própria.
- Alternativas comparadas: (1) calcular somente no frontend a partir do agregado, esforço baixo mas sem histórico; (2) criar endpoint/série paralela, separado mas com duplicação de contrato e agregação; (3) acrescentar data/hora à consulta de snapshots, preencher os buckets diário, semanal e mensal já existentes e reutilizar o gráfico de barras. Escolhida a alternativa 3: menor acoplamento adicional, uma fonte de verdade e preservação do dia operacional de 02h–02h.
- Implementação: o repositório agora busca `createdAt` e `quotaUsage`; o serviço filtra a janela Codex semanal (10.080 minutos), ignora JSON inválido e janelas diferentes, agrega p.p. nos buckets operacional diário e calendários semanal/mensal. A ausência de medição válida fica como `null`, não como `0`. O dashboard usa esse campo para criar os dois gráficos `Cota semanal consumida`; barras sem medição são neutras e informam `Indisponível` no tooltip. A referência tracejada e rotulada `Meta diária: 14 p.p.` é exibida só no painel diário para não sugerir uma meta semanal incorreta.
- Proteção contra regressão: o teste de serviço passou a validar a projeção de 1,25 p.p. em buckets diário, semanal e mensal, inclusive o descarte de JSON inválido. O E2E do dashboard valida os dois gráficos, o valor em p.p. e uma única referência diária.
- Homologação local: `mvn -q -Dtest=CodexRequestServiceTest test` e `mvn -q test` aprovados; `npm ci --no-audit --no-fund`, `npm run lint`, `npm run build` e o Playwright direcionado passaram. A captura desktop `/tmp/ai-hub-dashboard-token-charts.png` e a emulação Playwright de iPhone 15 Pro `/tmp/ai-hub-dashboard-quota-mobile.png` foram inspecionadas e confirmaram barras, rótulos e referência sem sobreposição. Os avisos de Browserslist desatualizado e bundle acima de 500 kB são preexistentes e não bloqueiam esta mudança.
- Entrega: alterações permanecem apenas na worktree; nenhum commit, push, Pull Request, pipeline, deploy ou publicação foi realizado.

## 2026-09-18 — Orientação de entrega até merge e deploy

- Solicitação: substituir o encerramento em código local/PR pendente pela execução, revisão/aprovação permitida, merge na main, acompanhamento e correção dos workflows, com conclusão somente após deploys bem-sucedidos.
- Pergunta explícita: **“por que esse erro aconteceu?”** O frontend (`CodexChatgptPage.tsx`) e os dois caminhos do orquestrador (`jobProcessor.ts`, App Server/Responses API) injetavam proibições de abrir PR sem novo pedido e orientavam aguardar deploy; a retomada pós-PR só mandava relatar pendência de aprovação. Não bastava adicionar uma frase ao prompt.
- Alternativas: (1) acrescentar uma instrução final, baixo esforço mas contradições mantidas; (2) substituir as instruções conflitantes em cada ponto de composição, esforço moderado e aderência direta ao pedido; (3) implementar coordenador persistente com webhooks/estado de deploy, esforço e risco elevados para um pedido de orientação. Escolhida (2), mantendo o coordenador como escopo distinto e sem alegar garantia mecânica de deploy.
- Antes dos testes, definida a matriz em `docs/entrega-modelo-github.md`: emissão dos prompts Managed/MKT, App Server/Responses, retomada, falhas e bloqueios, exceções de análise/local/Sandbox, evidências por SHA, segregação e Chromium desktop/mobile.
- Referência consultada: documentação oficial OpenAI sobre instruções (`https://learn.chatgpt.com/docs/agent-configuration/agents-md`); a inspeção local identificou os prompts efetivos deste produto.
- Implementação: módulos de orientação autocontidos no frontend e orquestrador substituem proibições antigas; Managed/MKT e demais perfis com repositório recebem o critério de entrega, enquanto Sandbox sem Git preserva seu contrato. A continuação pós-PR consulta revisões reais e prossegue com a entrega. A política mantém validação local, restrições explícitas, proteção de branch, imagem versionada e criação de artefatos pelo frontend após deploy.
- Homologação: `npm test` do orquestrador aprovado (150/150); build TypeScript/Vite e lint do frontend aprovados; Playwright direcionado aprovado (7/7), com Managed, MKT e Sandbox em desktop e iPhone 15 Pro emulado, além da continuação pós-PR. Provedores e APIs foram simulados; nenhuma publicação foi usada como teste.
- Ajuste iterativo dos testes: o primeiro mock abrangente `**/api/**` interceptava também `/src/api/client.ts` do Vite e impedia o carregamento da tela; corrigida a causa com regex ancorada no endpoint `/api/` da origem local. Corrigida também variável não utilizada na configuração mobile apontada pelo lint. A repetição dos cenários afetados passou. Avisos preexistentes de Browserslist e tamanho do bundle permanecem não bloqueantes.
- Revisão final: políticas equivalentes entre frontend/orquestrador, ausência das proibições antigas nos prompts ativos, diff sem problemas de whitespace e artefato transitório do Playwright restaurado. A mudança de orientação está validada localmente. Foi solicitada confirmação sobre aplicar a nova política à publicação desta própria alteração, pois a instrução anterior reservava PR a pedido explícito; sem resposta até este registro, não houve commit, push, PR, merge ou deploy.

## 2026-09-18 — Corrigir PR automático sem commits entre base e branch

- Solicitação: corrigir as duas falhas `422 No commits between main and ai-hub/codex-paulofor-marketing-hub-main-chatgpt_codex_mkt`.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** `maybeCreatePullRequest` usava o patch relativo ao SHA inicial da base como condição suficiente para criar PR; `createOrReusePullRequest` fazia POST antes de consultar PR existente e, no 422, só procurava PR aberto. Uma branch já integrada, atrás da main ou integrada durante a execução continuava produzindo patch histórico, mas não tinha commits remotos para integrar. A consulta somente leitura ao GitHub confirmou `ahead_by=0`, `behind_by=4`, `files=[]` na branch informada; a reprodução local retornou o mesmo 422 nos oito perfis com repositório.
- Alternativas comparadas: (1) capturar só a mensagem 422, baixo esforço, mas mantém a requisição inválida e arrisca ocultar falhas; (2) comparar Git local após fetch, esforço médio e complicações com clone raso/base concorrente; (3) consultar PR e comparação remota antes do POST, reconfirmando o 422 específico em caso de corrida, esforço médio e mesma fonte de verdade usada na publicação. Escolhida (3). Matriz definida antes dos testes em `docs/homologacao-pr-sem-commits.md`.
- Implementação: reutilizar PR aberto antes de criar; consultar comparação atual base/head e dispensar PR quando não houver commits novos ou arquivos alterados; classificar o erro 422 por status e payload estruturado e consultar novamente antes de aceitar um merge concorrente. Outros 422, falhas de consulta/autenticação/transporte e respostas inválidas continuam falhando e preservando workspace. O patch histórico permanece disponível e os logs distinguem ausência de novidades e reutilização. A lógica é comum a todos os perfis com repositório.
- Homologação: reprodução anterior à correção confirmou 11 falhas direcionadas, incluindo o erro reportado. Após a mudança, 33/33 testes de publicação/preservação passaram. Acrescentado caso de branches idênticas e executada uma rodada completa: `npm test` do orquestrador passou com **177/177**, sem falhas ou skips. Cobertura inclui oito perfis, PR existente, merge durante o job, corrida de PR/merge, código novo, ausência de diferença líquida, 422 não confirmado, 401/403, JSON inválido, rede indisponível, push rejeitado, validação local falhando e publicação automática desativada.
- Verificação adicional: TypeScript compilado pelo npm test; `bash -n`, `shellcheck` e o validador versionado de 20 scripts passaram; `git diff --check` passou. Sem alterações de frontend ou contrato público: validação de navegador não se aplica. Git bare e provedores simulados, dados sintéticos, sem chamadas a modelos pagos nem escrita no Marketing Hub; consultas reais foram somente leitura.
- Referências oficiais consultadas: `https://docs.github.com/en/rest/pulls/pulls#create-a-pull-request` e `https://docs.github.com/en/rest/commits/commits#compare-two-commits`. A solução foi revisada localmente antes de qualquer commit/publicação. Entrega autorizada pelo usuário: PR único desta correção, revisão sem autoaprovação, checks do HEAD, merge na main, acompanhamento de CI/build/deploy e verificação de saúde. Liquibase não se aplica porque nenhum changelog ou arquivo do seu filtro foi alterado; build/deploy só se aplicam à main conforme condição do CI.
- Iteração após a validação inicial: aberto e revisado o [PR #726](https://github.com/paulofor/ai-hub/pull/726). O [primeiro CI](https://github.com/paulofor/ai-hub/actions/runs/35384994826) passou backend/frontend/MCP, mas expôs diferença real do runner: sem `rg`, o preflight criava `.ai-hub-bin/rg`, incluído indevidamente pelo `git add -A`. **Por que esse erro aconteceu?** A exclusão de artefatos internos existia na coleta de arquivos não rastreados, mas não no estágio nem no patch de arquivos já staged. A sandbox tinha `rg` instalado, por isso a primeira rodada não exercitava a condição.
- Comparadas três alternativas: instalar `rg` só no CI (baixo esforço, mascara o defeito em outras máquinas), inserir `.gitignore` em cada repositório (esforço médio, invasivo e não resolve índice staged), ou alinhar patch e estágio às exclusões internas existentes (esforço médio, correção comum a todos os runners). Escolhida a terceira. Reproduzidas localmente as duas falhas representativas antes do ajuste; o estágio agora preserva os arquivos internos no disco, retira-os do índice e adiciona somente arquivos publicáveis, decidindo o commit pelo diff staged. Testes sempre exercitam o fallback real e incluem anexos já staged, sem depender dos pacotes do host; o script fallback passou `bash -n` e ShellCheck. A suíte direcionada passou com 35/35 após a correção. A atualização permanece no mesmo PR, antes de merge/deploy.
- Validação final da iteração: **178/178 testes aprovados** com compilação TypeScript e limpeza Docker desativada para testes sem containers; `git diff --check` e nova revisão do diff aprovados. Nenhuma reexecução cega do CI: o novo HEAD inclui a correção reproduzida e validada localmente.

### Confirmação da entrega e retomada da solicitação

- Na retomada após a mensagem de job indisponível, a correção já estava na `main`. **Por que o erro 422 aconteceu?** A comparação remota somente leitura da branch informada confirmou novamente `status=behind`, `ahead_by=0`, `behind_by=4`, `files=[]`; o patch histórico não representava commits novos publicáveis. A proteção implementada cobre essa condição sem ocultar erros reais.
- Alternativas avaliadas: (1) reimplementar a correção já integrada, esforço e risco desnecessários; (2) apenas informar o merge, esforço baixo mas sem comprovar testes e operação; (3) validar o código existente, conferir CI/deploy/saúde e complementar o diário, esforço baixo e evidência completa. Escolhida (3), sem nova alteração funcional nem commit artificial na branch do Marketing Hub.
- Entrega funcional confirmada: [PR #726](https://github.com/paulofor/ai-hub/pull/726), HEAD `1d600771f567e98e5a75204f460cb5f1e2162bd5`, merge em **18/09/2026 às 19:24:01 UTC**, SHA da `main` **`c42655ed432c66e7f5fe7a4ce5aeb7d64324341f`**. O [CI do HEAD do PR](https://github.com/paulofor/ai-hub/actions/runs/35385459939) passou; build e deploy são condicionados à main e, portanto, corretamente dispensados no PR. As revisões registradas são comentários, sem autoaprovação.
- [CI da main](https://github.com/paulofor/ai-hub/actions/runs/35385717533): backend, frontend, sandbox, MCP, construção das imagens e deploy concluídos com sucesso. `gh run watch --exit-status` confirmou sucesso; nova consulta por SHA confirmou ausência de execução pendente. Liquibase não se aplica aos arquivos alterados e não há workflow encadeado configurado.
- O [job de deploy](https://github.com/paulofor/ai-hub/actions/runs/35385717533/job/105733177603), concluído às **19:29:07 UTC**, registra os serviços em execução com imagens versionadas pelo SHA `c42655ed432c66e7f5fe7a4ce5aeb7d64324341f`, inclusive `ai-hub-6-sandbox`, e aprovação da etapa `Verify production services`. A consulta pública independente confirmou `https://iahub.xyz/` com HTTP 200 e `https://iahub.xyz/mcp` com HTTP 200 e `{"status":"UP"}`. A consulta SSH adicional somente leitura foi recusada pela autenticação; a versão foi comprovada pelo log autenticado da pipeline, sem contornar o helper nem publicar via SSH.
- Homologação da retomada na sandbox: `npm ci --no-audit --no-fund` e `DOCKER_HOMOLOGATION_CLEANUP_ENABLED=false npm test` aprovados, **178/178 testes**, zero falhas e zero skips. `bash -n`, `shellcheck`, o validador versionado de 20 scripts e `git diff --check` aprovados. Git bare local e APIs de publicação/modelos simulados, sem dados reais de marketing ou containers de homologação. A matriz já definida em `docs/homologacao-pr-sem-commits.md` permanece atendida; nenhuma mudança de UI exige rodada de navegador.
- Este complemento registra a conclusão da entrega existente; sua publicação é exclusivamente documental, pelo fluxo normal de PR, revisão, checks e merge, sem alterar a solução funcional validada.

## 2026-09-18 — Proteção complementar no pedido manual de PR

- Solicitação retomada após as duas falhas `422 No commits between main and ai-hub/codex-paulofor-marketing-hub-main-chatgpt_codex_mkt`.
...
## 2026-09-19 — Exibição dos objetivos publicados durante o raciocínio

- Solicitação: investigar por que os objetivos do modelo não apareciam no card “Resumo do raciocínio” e corrigir o fluxo.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** O Codex App Server publica o checklist de objetivos pelo evento público `turn/plan/updated`, separado dos eventos `item/reasoning/*`. O orquestrador assinava apenas os eventos de resumo de raciocínio e descartava o evento de plano; além disso, a orientação textual adicionada ao prompt não consegue garantir o formato de um resumo automático produzido pelo provedor. Assim, a interface recebia exatamente os títulos vistos na captura, mas nunca recebia os objetivos estruturados.
- Correção na causa: o coletor agora aceita atualizações públicas de plano, converte os passos em checklist Markdown e mantém apenas a versão mais recente de cada turno. O listener do App Server associa o evento à thread correta, atualiza a atividade do job e incorpora a seção `Objetivos` ao mesmo campo público já persistido e exibido pelo frontend, sem coletar raciocínio interno.
- Proteção contra regressão: teste de integração do orquestrador simula `turn/plan/updated` junto com um resumo público, verifica os estados concluído/pendente, a explicação do plano e a preservação do resumo. A compilação TypeScript e o teste direcionado passaram. A suíte completa executou 179 testes, com 177 aprovações e duas falhas ambientais preexistentes porque o executável `shellcheck` não está instalado; nenhum teste de resumo ou objetivo falhou.

## 2026-09-20 — Objetivos ausentes mesmo após a coleta do plano

- Solicitação: investigar por que a solicitação #3009 ainda mostrava somente títulos no card “Resumo do raciocínio”, sem a seção de objetivos.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** A correção anterior passou a coletar corretamente o evento público `turn/plan/updated`, mas o evento só existe quando o modelo chama a tool `update_plan`. Na execução real #3009, iniciada depois daquela entrega, o prompt apenas sugeria um formato para o resumo automático e o modelo não publicou plano algum; por isso não havia objetivos estruturados para o coletor persistir. A orientação de texto não controla o conteúdo que o provedor produz em `summary: "auto"`.
- Correção na causa: o prompt exclusivo do Codex App Server agora exige `update_plan` antes da primeira ação, atualização do mesmo checklist quando o escopo mudar e conclusão das etapas. A instrução também deixa explícito que títulos do resumo automático não substituem o plano público. O caminho Responses API não recebe essa exigência porque não dispõe dessa tool.
- Proteção contra regressão: o teste do payload `turn/start` do perfil MKT passa a verificar as duas partes essenciais da instrução: publicação obrigatória do checklist de objetivos por `update_plan` e proibição de tratá-lo como equivalente aos títulos automáticos.

## 2026-09-20 — Impacto dos objetivos na performance do modelo

- Pergunta recebida: a publicação dos objetivos ajuda a entender e melhorar a performance do modelo?
- Resposta: ajuda diretamente na observabilidade e no diagnóstico, porque torna explícito o plano público escolhido, permite comparar intenção, execução e conclusão e evidencia mudança de escopo ou etapa abandonada. Também pode melhorar indiretamente a execução de tarefas longas ao manter um checklist atualizado.
- Limite: a mudança não melhora por si só a capacidade do modelo nem comprova ganho de qualidade, velocidade ou custo. A chamada adicional a `update_plan` consome poucos tokens e pode acrescentar pequena latência; em solicitações simples, seu benefício tende a ser menor.
- Para comprovar melhoria de performance, ainda é necessário correlacionar a presença e a atualização do plano com métricas já registradas, como duração, tokens, custo, timeouts, reaberturas, avaliação e conclusão do objetivo, idealmente comparando grupos equivalentes antes e depois da mudança.

## 2026-09-20 — Análise das solicitações 3003 e 3011: ações concluídas e falha no encerramento

- Solicitação: analisar por que #3003 e #3011 deram erro embora aparentemente tenham sido executadas. Escopo desta execução: diagnóstico, consultas de leitura, reprodução local e registro documental; nenhuma campanha, orçamento, status histórico, código funcional ou publicação foi alterado.
- Planejamento: a ferramenta `update_plan` foi procurada entre as ferramentas expostas e não está disponível nesta sessão. O checklist público equivalente foi informado e atualizado: localizar registros, comparar execução/erro/efeitos e registrar o diagnóstico, sempre com o objetivo de cada etapa.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** Os dois registros têm `status=FAILED` e a mesma resposta: a branch `ai-hub/codex-paulofor-marketing-hub-main-chatgpt_codex_mkt` avançou ou divergiu. Não se trata do antigo `422 No commits between`: esta exceção ocorre antes da consulta de comparação/criação do PR que foi corrigida no PR ai-hub #726.
- Alternativas de investigação comparadas: (1) consultar apenas o estado atual e os PRs, baixo esforço e boa prova do efeito, mas sem explicar o encerramento; (2) recuperar eventos públicos das sessões arquivadas e correlacioná-los com APIs e GitHub, esforço moderado e cronologia verificável; (3) reproduzir o encerramento com Git local e modelo simulado, esforço moderado e diagnóstico determinístico, mas insuficiente isoladamente para provar a ação produtiva. Escolhida a combinação de (2) e (3), aproveitando as consultas de (1), sem repetir operações comerciais.

### Resultados comerciais e cronologia confirmados

| Solicitação | Trabalho solicitado e evidência | Resposta final original | Registro de falha |
| --- | --- | --- | --- |
| 3003 | Colocar o experimento #92 em RUNNING. A API confirma RUNNING e campanha `120251742861340326`; métricas sincronizadas em 20/09 às 09:50:44 UTC registravam 165 impressões, 4 cliques e R$ 9,26 gastos. | Em 19/09 às 22:15:12 UTC, o modelo informou “Vega #92 está em RUNNING”, campanha/conjunto/anúncios ativos, v8 pública e entrega validada. | Em 19/09 às 22:15:15 UTC, cerca de 3 segundos depois da conclusão do modelo. |
| 3011 | Reativar o #91 com limite de R$ 150. A API confirma RUNNING, `mediaSpendLimit=150`, `zeroResultSpendLimit=150`, prazo até 26/09 e ação `META_RESUME_CONFIRMED`. O pedido de retomada #1 foi solicitado às 06:11:10 e concluído às 06:12:14 UTC de 20/09, com readback Meta ACTIVE e orçamento vitalício de 15000 centavos. | Em 20/09 às 06:20:36 UTC, o modelo informou “Vega #91 reativado”; evento de conclusão às 06:20:37 UTC. O teto é acumulado: R$ 27,45 anteriores e R$ 122,55 disponíveis na retomada. | Em 20/09 às 06:20:40 UTC, cerca de 3 segundos depois do evento de conclusão. |

- Evidências da #3003: [PR #5265](https://github.com/paulofor/marketing-hub/pull/5265), [PR #5266](https://github.com/paulofor/marketing-hub/pull/5266) e [PR #5268](https://github.com/paulofor/marketing-hub/pull/5268), integrados durante a execução. SHA final `351b46ceee4b7a7e36c7dfd8037b3c09de54b70d`; [deploy da aplicação](https://github.com/paulofor/marketing-hub/actions/runs/35471387151) e [reconciliação final](https://github.com/paulofor/marketing-hub/actions/runs/35472607486) confirmados como `success`. O relato original ainda tinha zero impressões/gasto; as métricas posteriores comprovam que houve veiculação desde então.
- Evidências da #3011: [PR #5270](https://github.com/paulofor/marketing-hub/pull/5270), integrado em 20/09 às 05:53:53 UTC, SHA `660be280d140d783de07222b71e0d642eadebd5f`; [deploy da aplicação](https://github.com/paulofor/marketing-hub/actions/runs/35492830204), [deploy do Facebook Ads Worker](https://github.com/paulofor/marketing-hub/actions/runs/35492830250) e [verificação final](https://github.com/paulofor/marketing-hub/actions/runs/35493871945) confirmados como `success`. O registro de retomada preserva a campanha `120251556536430326`, orçamento, prazo e confirmação nativa da ativação.
- Fontes operacionais: `GET https://iahub.xyz/api/codex/requests/{3003,3011}`, `GET http://191.252.181.168:8000/api/experiments/{91,92}` e `GET /api/facebook-campaign-resumptions/experiments/91` no mesmo backend. Nenhum endereço de e-mail, token ou payload de credenciais foi incluído neste diário.

### Causa técnica e reprodução local

- O modelo já havia publicado por branches próprias, concluído os PRs/deploys e produzido a resposta final. Porém `SandboxJobProcessor.process` salva `job.summary` e depois chama incondicionalmente a finalização Git quando existe patch relativo ao SHA inicial. Esse patch pode representar trabalho já integrado. `maybeCreatePullRequest` tenta voltar à branch compartilhada e republicá-la, sem primeiro reconhecer a entrega já concluída pelo modelo.
- `cloneRepository` usa `git clone --depth 1`. A checagem em `checkoutWorkBranch` interpreta qualquer falha de `git merge-base --is-ancestor origin/<workBranch> HEAD` como avanço/divergência remota. Em um histórico raso, a ausência da cadeia de ancestrais também produz retorno 1, mesmo que a branch esteja totalmente integrada. O log da #3003 contém explicitamente `grafted` no SHA inicial `28c5aec44edddc71f0832800d94372aec4f02056`. As sessões registram criação de branches a partir de `origin/main`, sem expansão do histórico. Na #3003 o modelo terminou em main; na #3011, na branch de entrega já integrada pelo PR #5270.
- A comparação do GitHub com histórico completo confirma ancestralidade, sem commits exclusivos do lado antigo: `089ce81ab74079c482b4dc4b16e4086ddb19f9e6...351b46ceee4b7a7e36c7dfd8037b3c09de54b70d` retorna `ahead_by=67`, `behind_by=0`; `b4627dcbbc5d236e905fa96b7d34025a9c34b504...039f01dd9e589d30f87ad6fb562d08ac2ff95a8b` retorna `ahead_by=6`, `behind_by=0`. Na consulta atual, a branch compartilhada está atrás da main, com `ahead_by=0`, `behind_by=8` e `files=[]`.
- A exceção de encerramento muda o job para `FAILED`. No backend, `CodexRequestService.resolveUserFacingResponseTextFromSandbox` prioriza `response.error()` sobre `response.summary()`: a mensagem de erro ocupa a resposta visível, ocultando o relatório de sucesso já produzido. A exceção não desfaz PR, deploy, ativação Meta ou orçamento previamente confirmados. `pullRequestUrl=null` nos registros do AI Hub também não prova ausência de PR criado pelo modelo.
- Validação na sandbox: `npm ci --include=dev --no-audit --no-fund` e compilação TypeScript aprovados. Um reproducer temporário usa a classe real `SandboxJobProcessor`, repositórios Git bare via `file://` (para respeitar `--depth 1`), modelo simulado e nenhuma chamada externa. Três cenários confirmados: (1) entrega já integrada + histórico raso gera exatamente a falha reportada, apesar de ancestralidade válida no remoto e branch inalterada; (2) o mesmo fluxo com histórico completo termina `COMPLETED`; (3) divergência real com histórico completo continua sendo bloqueada. Os repositórios sintéticos foram removidos ao final. Não houve execução de modelo pago, container ou publicação para testar.
- Evidências temporárias da reprodução: `/tmp/aihub-analysis-3003-3011/reproduce-finalization.mjs` e `reproduction-results.json`. O código funcional examinado está em `apps/sandbox-orchestrator/src/jobProcessor.ts` e `apps/backend/src/main/java/com/aihub/hub/service/CodexRequestService.java`. As respostas finais públicas foram recuperadas das sessões arquivadas `01a0baf5-801b-78c0-97df-d94d840788d9` e `01a0bd1f-78a9-7382-b94f-7070b237fce1`, sem registrar raciocínio interno.

### Direção de correção e limites

- Alternativas de correção avaliadas: (1) completar o histórico antes da checagem, esforço baixo e correção da falsa divergência, mas mantém a tentativa redundante de publicar; (2) reconhecer por evidências Git/GitHub a entrega já integrada e preservar a resposta do modelo separada do erro de finalização, esforço moderado e aderência ao fluxo atual; (3) criar estados persistentes separados para execução comercial e publicação, esforço maior e contrato de UI/backend mais amplo. Direção recomendada: (2), com checagem de ancestralidade confiável de (1), preservando bloqueio de divergências reais e trabalho local ainda não publicado. Não converter genericamente qualquer `FAILED` em sucesso.
- Limites de observabilidade: os dois jobs já retornam 404 no orquestrador e o download de interações retorna lista vazia apesar dos contadores históricos; as sessões arquivadas permitiram recuperar a cronologia. SSH operacional no host do AI Hub foi recusado por autenticação e não foi contornado. A consulta direta atual à Meta com a credencial disponível retornou 401; a comprovação da ativação usa o readback persistido da operação, as respostas públicas originais e o estado/métricas atuais sincronizados pelo worker, sem alegar nova leitura direta bem-sucedida da Meta.
- Conclusão: os objetivos operacionais das duas solicitações foram executados; o erro ocorreu no encerramento automático do AI Hub e na apresentação da resposta. Repetir a ativação não é necessário. A única alteração versionada desta análise é este registro documental, mantido localmente; nenhum novo PR, commit, push ou deploy foi realizado.

## 2026-09-20 — Habilitação explícita de update_plan nas sessões Codex

- Solicitação: disponibilizar a ferramenta `update_plan` para publicar e atualizar o checklist. A sessão que executa esta correção não expõe a ferramenta no catálogo; o plano público foi comunicado sem simular uma chamada inexistente. Etapas: investigar a causa; corrigir e homologar localmente; revisar e entregar via PR, merge e workflows.
- Pergunta explícita: **“por que esse erro aconteceu?”** O prompt do App Server exige `update_plan`, mas `thread/start` não habilita a ferramenta. O [changelog oficial OpenAI](https://learn.chatgpt.com/docs/changelog) registra que o Codex 0.152.0 tornou o plano opt-in por `tools.update_plan.enabled = true`; o binário local é 0.153.4. O listener de `turn/plan/updated` e o coletor público já existem e, sem a ferramenta habilitada, não recebem eventos. Acrescentar instruções ao prompt não registra uma ferramenta desabilitada.
- Alternativas: (1) habilitar por `thread/start.config`, baixo esforço, escopo restrito às threads do AI Hub e aderência direta; (2) configurar o processo/global, baixo esforço mas dependente da configuração persistente do host e com alcance sobre outros clientes; (3) criar ferramenta dinâmica equivalente, maior esforço e risco de duplicar o contrato, eventos e manutenção do Codex. Escolhida (1), mantendo a ferramenta e o coletor nativos.
- Matriz definida antes dos testes: três perfis (`CHATGPT_CODEX`, `CHATGPT_CODEX_MKT`, `CHATGPT_CODEX_SANDBOX`) recebem opt-in antes de `turn/start`; contrato e auditoria enviam os mesmos parâmetros; publicação inicial e atualização substituem o checklist do turno; eventos de outra thread e raciocínio bruto não contaminam a resposta; falha preserva o plano já publicado; integração por processo JSON-RPC chega ao polling HTTP com resposta final e métricas preservadas. Verificação adicional com Codex real e provedor local simulado, sem modelo pago nem campanhas/dados produtivos. O frontend mantém o contrato Markdown existente; não há layout ou interação nova que requeira ampliar a matriz de navegadores.
- Limite de ativação: a configuração passa a valer nas threads criadas pelo orquestrador atualizado, inclusive a próxima solicitação desta conversa. Ela não altera retroativamente o catálogo de ferramentas do turno já iniciado.
- Implementação: `thread/start` recebe `config: { "tools.update_plan.enabled": true }` nos três perfis Codex; o mesmo objeto é enviado ao App Server e ao registro de auditoria para evitar divergência. A documentação explica o limite de ativação em novas execuções.
- Validação local: os três testes de contrato falharam antes da correção por opt-in ausente; depois, a suíte completa passou com **180/180 testes**, sem falhas ou skips. O teste de processo JSON-RPC confirma duas atualizações, checklist final único, resumo e resposta final preservados no polling HTTP, cota sem leitura simulada e nenhum timeout. Casos adicionais cobrem plano durante execução, isolamento entre threads, eventos inválidos e preservação em falha. `bash -n` e ShellCheck aprovaram os 20 scripts do repositório; `git diff --check` aprovado.
- Verificação com runtime real: `/tmp/aihub-update-plan/smoke.mjs` iniciou Codex 0.153.4 com diretório temporário e provedor Responses/SSE restrito a `127.0.0.1`, sem credenciais reais. Sem a configuração: ferramenta ausente e zero eventos. Com configuração por thread: ferramenta presente, chamada inicial e atualização concluída geram dois eventos `turn/plan/updated`; resposta nativa `Plan updated` confirmada. Uma terceira chamada com status inválido foi rejeitada sem sobrescrever o plano. O processo e diretório temporário foram removidos.
- Revisão local: alteração funcional limitada ao opt-in por thread; sandbox/approvalPolicy, autenticação, resumo final e contratos HTTP preservados. PR existente **#732** será atualizado com esta correção e com o registro anterior já presente; entrega pela pipeline versionada, sem publicação direta por SSH. Checklist: investigação concluída; implementação/homologação concluídas; entrega em andamento.

## 2026-09-20 — Correção do encerramento e do histórico das solicitações 3015, 3016 e 3017

- Solicitação: investigar três execuções marcadas como falha apesar de código entregue, corrigir a causa e a informação exibida. Checklist publicado com `update_plan` antes da investigação e atualizado após diagnóstico e homologação. Objetivo: distinguir resultado real de erro de encerramento e evitar reexecução de efeitos comerciais.
- Pergunta explícita: **“por que esse erro aconteceu?”** As três solicitações registraram `FAILED` com a mesma falsa divergência da branch compartilhada, entre 1,5 e 1,7 segundo após o evento público de conclusão. Seus workspaces preservados confirmam `--is-shallow-repository=true` e retorno 1 de `merge-base --is-ancestor` para a branch remota. O GitHub, com histórico completo, confirma `behind_by=0` e respectivamente 25, 26 e 31 commits à frente: a ancestralidade era válida.
- A segunda causa está no backend: ao receber `summary` e `error`, ele exibia somente `error`, ocultando a resposta final. O patch histórico relativo ao início do job também era tratado como trabalho ainda não publicado, mesmo depois do PR integrado.

| Solicitação | Resultado comprovado | Final público / erro (UTC) | PR, SHA e verificação |
| --- | --- | --- | --- |
| 3015 | Subprocesso Quartzo de Capella implementado; 4/8 etapas preparadas. Divulgação continuava dependente de informações de margem, como a resposta original informa. | 16:49:05.788 / 16:49:07.376 | [5274](https://github.com/paulofor/marketing-hub/pull/5274), `30213339533abbbfcaf7655d7420ebdcdeecdf1b`; [backend](https://github.com/paulofor/marketing-hub/actions/runs/35522450912), Psique 35523379200, Têmis 35523379244 e verificação 35523696996, todos success. |
| 3016 | WatchDog corrigido para medir progresso da fila e evitar falso alerta. | 17:15:25.798 / 17:15:27.431 | [5275](https://github.com/paulofor/marketing-hub/pull/5275), `fc93345536ea3fa61e493919c22d3aee5857abfc`; contrato 35524622065, central 35524622046, Customer Agent 35524662092, Meta Worker 35524662108 e [WatchDog](https://github.com/paulofor/marketing-hub/actions/runs/35525185581), todos success. |
| 3017 | Acesso ao processo corrente de Capella corrigido. | 17:56:37.774 / 17:56:39.237 | [5276](https://github.com/paulofor/marketing-hub/pull/5276), `5fdc14c9e6361fa9a06c79e3c280f7de316c5ed9`; [deploy](https://github.com/paulofor/marketing-hub/actions/runs/35526479624) e [WatchDog](https://github.com/paulofor/marketing-hub/actions/runs/35527006049), ambos success. |

- Fontes: API `/api/codex/requests/{id}`, eventos `task_complete.last_agent_message` das sessões arquivadas, workspaces preservados em modo somente leitura, PRs e 11 runs consultados via GitHub. Somente respostas finais públicas foram recuperadas, sem raciocínio interno. Os HEADs preservados são `aacdae89d2955ead50c4211b78f091733b1f302d`, `7bbab7293bb564edf3c662af2c8e5b5224557dfe` e `b60163e8697e7a2b469adae76acaca01a6c30253`; todos descendem da branch compartilhada `b4627dcbbc5d236e905fa96b7d34025a9c34b504`. Os dois primeiros PRs usaram squash, motivo para reconhecer também equivalência exata da árvore de código.
- Alternativas comparadas: (1) completar histórico, baixo esforço, corrige a falsa divergência mas mantém publicação redundante; (2) reconhecer a entrega por evidências Git e preservar resposta/erro, esforço moderado e aderência direta; (3) novos estados persistidos de execução/publicação, maior custo e mudança de contrato. Escolhida a combinação 1 + 2. Não se converte genericamente `FAILED` em sucesso.
- Implementação: antes de republicar, exigir ausência de alterações staged/unstaged/untracked e confirmar que o HEAD já pertence à base remota atual ou que a árvore de código é idêntica (squash/rebase). Caso contrário, seguir a publicação normal, completando o histórico antes de concluir divergência. Consulta, expansão de histórico, push e teste com falha continuam preservando workspace e `FAILED`. Anexos e ferramentas internas continuam excluídos da publicação.
- Backend: resposta final permanece visível junto do aviso de erro no encerramento; JSON MKT mantém seus campos, com aviso dentro de `comentario`. O status de falha real continua explícito e callback repetido não duplica aviso ou tokens.
- Matriz definida antes da implementação em [homologacao-encerramento-solicitacoes.md](../homologacao-encerramento-solicitacoes.md). Código original reprovou quatro cenários Git e dois de preservação da resposta. Validação corrigida: suíte completa do orquestrador 187/187, depois regressão final ampliada 45/45; backend 147/147 e integração adicional 2/2; seis verificações Playwright com respostas persistidas pelo backend em desktop, iPhone 15 Pro e Pixel 7; lint/build frontend; `bash -n` + ShellCheck nos 20 scripts; revisão do diff. Fixtures usam Git bare local, modelo/GitHub simulados, H2 e `sandbox.local`; nenhum modelo pago, campanha ou métrica produtiva foi usado para teste.
- Reconciliação validada em dry-run e aplicada aos IDs 3015/3016/3017: conferidos externalId e mensagem exata, final público anterior ao erro, PR integrado e 11 runs do SHA correspondente com sucesso. Atualização em transação com bloqueio de linha, backup local protegido e auditoria em `execution_log`; restaurados somente status, resposta original e URL do PR, mantendo tokens, custos e datas. Readback da API confirmou os três `COMPLETED`, títulos originais e links dos PRs corretos. Nenhuma ação comercial foi repetida.
- Revisão de entrega: nenhuma mudança de schema/changelog, autenticação, sandbox ou pipeline; Liquibase não se aplica pelos filtros de paths. Publicação segue PR, revisão local, checks do HEAD e merge sem bypass ou aprovação do próprio autor. Evidências temporárias locais em `/tmp/aihub-3015-3017`.

## 2026-09-21 — Entrega até o deploy e preservação dos vínculos de PR

- Solicitação: executar, revisar/aprovar quando permitido, integrar na main, acompanhar Actions/deploys e corrigir falhas até concluir, preservando a informação correta das solicitações 3015/3016/3017. Checklist publicado por `update_plan` antes da investigação e atualizado conforme os achados. Objetivo: entregar com evidências e sem republicar trabalho concluído.
- Estado confirmado: a política básica já veio do [PR #725](https://github.com/paulofor/ai-hub/pull/725); a correção do falso FAILED está no [PR #733](https://github.com/paulofor/ai-hub/pull/733), SHA `902aecea10551ad6e997af0eeb6a6a7ee7e502c0`, com os seis jobs, inclusive imagens/deploy, aprovados no [run 35535344373](https://github.com/paulofor/ai-hub/actions/runs/35535344373). As revisões reais consultadas dos PRs 725, 733 e Marketing Hub 5281 são vazias: não se afirma aprovação inexistente. A identidade disponível é a do autor; nenhuma autoaprovação ou bypass é usado.
- O [PR Marketing Hub #5281](https://github.com/paulofor/marketing-hub/pull/5281) já foi integrado em `0412d002b61841a09ac5b756416b5a950c6336e7`. Seu escopo é produto/financeiro no Marketing Hub; a orientação e o histórico pertencem ao AI Hub. O lote existente não é recriado; seus checks e deploys seguem acompanhados, incluindo o [deploy central 35557258341](https://github.com/paulofor/marketing-hub/actions/runs/35557258341). Watchdogs cancelados foram substituídos por novas execuções para o mesmo SHA conforme `concurrency.cancel-in-progress`; o sucesso substituto precisa ser confirmado, sem confundir cancelamento com aprovação.
- Pergunta explícita: **“por que esse erro aconteceu?”** Os três estados já estavam `COMPLETED`, mas a API e o banco mostravam indevidamente PR 5209 em todos. A auditoria preservada identifica corretamente os PRs 5274/5275/5276. `listBatch` tratava a branch reutilizável como identidade eterna de lote quando a solicitação já tinha PR; `createPr` escolhia o primeiro vínculo antigo e `markPullRequestCreatedForBatch` o gravava em todos, inclusive entregas reconciliadas. Uma solicitação com `FAILED` e PR também podia ser sobrescrita. Quatro testes reproduziram essas falhas antes da correção. Objetivo: corrigir a associação na origem, sem converter falhas reais em sucesso.
- Alternativas: (1) apenas reforçar o prompt, baixo custo e aderência parcial, sem proteger dados; (2) reforçar a política e tornar a associação de PR idempotente, esforço moderado, protege o histórico e mantém o contrato existente; (3) criar coordenador persistente de entrega/novos estados, esforço e risco maiores, com migrações e mudança ampla. Escolhida (2), preservando também as proteções Git já publicadas no #733.
- Implementação: priorizar o PR da própria solicitação; isolar solicitações já vinculadas da branch compartilhada; fechar somente itens sem PR, independentemente do status anterior; rejeitar substituição de vínculo existente com HTTP 409. A orientação no frontend e nos prompts App Server/Responses exige revisões reais, reconhecimento de entrega já integrada e distinção entre falha de execução, deploy pendente e erro posterior de encerramento. As instruções continuam sendo orientação do modelo, sem alegar que o término de um turno comprova o deploy. Referência de contrato: [documentação oficial OpenAI App Server](https://learn.chatgpt.com/docs/app-server).
- Interface: substituído texto antigo que exigia pedido separado de PR; ocultada a ação de criar outro PR quando já existe vínculo e desabilitada criação em solicitação não concluída. Cabeçalho e comentários agora quebram linha no celular, evitando o transbordamento visto na inspeção visual. Objetivo: mostrar a mesma política vigente e impedir ações redundantes.
- Matriz definida antes dos testes e ampliada em [entrega-modelo-github.md](../entrega-modelo-github.md): prompts Managed/MKT/Sandbox, retomada, restrições, falhas reais, associações distintas, repetição, conflito, métricas, API/H2/interface e desktop/mobile. Fixtures segregadas em `test/delivery` e `sandbox.local`, Git/modelos simulados; nenhum modelo pago ou campanha usado em teste.
- Validação local: orquestrador **190/190**; backend **153/153**, incluindo quatro regressões e a integração HTTP → H2 → detalhes da interface; **15 cenários Playwright** atendidos. Na rodada paralela, 14 passaram e um Chromium encerrou com `Target crashed`; o cgroup registrava OOM/um processo eliminado. O único caso afetado passou isolado com um worker. Depois do ajuste visual, somente os oito cenários de detalhe/encerramento foram repetidos, todos aprovados. Capturas desktop/iPhone inspecionadas; falhas também cobertas em Pixel 7. Lint/build, igualdade das políticas frontend/orquestrador, `bash -n` e ShellCheck nos 20 scripts e revisão do diff aprovados. Um erro de tipo na fixture (`Integer` de tokens) foi corrigido antes da execução completa do backend.
...
## 2026-09-23 — Diagnóstico da solicitação 3054

- Solicitação: verificar o que aconteceu com a execução 3054. Escopo restrito a consultas de leitura e registro do diagnóstico; nenhuma campanha, processo, estado histórico ou código funcional foi alterado.
- Pergunta explícita de causa raiz: **“por que esse erro aconteceu?”** A execução não terminou por falha dos ajustes implementados nem pelo antigo erro de encerramento Git. Depois de 4h05min25s e 6.310 interações, a última resposta do provedor foi `Selected model is at capacity. Please try a different model.`; o AI Hub persistiu corretamente `FAILED`, sem resposta final e sem PR vinculado à solicitação. O consumo registrado foi 104.106.173 tokens totais, dos quais 101.910.656 vieram do cache.
- Antes da indisponibilidade do modelo, a execução concluiu e integrou duas entregas no Marketing Hub. O PR [#5310](https://github.com/paulofor/marketing-hub/pull/5310), merge `733a6945cdbdbe8353c52686e348b79a90323180`, operacionalizou as quatro atividades do preflight e preservou o teto Meta; o PR [#5311](https://github.com/paulofor/marketing-hub/pull/5311), merge `0d4748c40de347eb46c472efd21999ae9381f52d`, corrigiu a incompatibilidade confirmada pela Meta entre orçamento diário e teto vitalício no mesmo conjunto, migrando o orçamento diário para a campanha e preservando o teto autorizado no conjunto.
- Os checks principais do merge final #5311 concluíram com sucesso: [Build & Deploy containers](https://github.com/paulofor/marketing-hub/actions/runs/35886080521), [Facebook Ads Worker CI](https://github.com/paulofor/marketing-hub/actions/runs/35886080557), [Meta Ad Approver Worker](https://github.com/paulofor/marketing-hub/actions/runs/35886080733), [Customer Agent Worker](https://github.com/paulofor/marketing-hub/actions/runs/35886080652) e [Production Freshness Watchdog](https://github.com/paulofor/marketing-hub/actions/runs/35892135863). Portanto, houve entrega real apesar do estado final `FAILED` da solicitação.
- O checklist preservado na execução indica que contexto, causa, preflight, quatro atividades e correção orçamentária foram concluídos. Restava terminar a revisão operacional, acompanhar a retomada da campanha e produzir a resposta final. A indisponibilidade ocorreu justamente durante esse acompanhamento; por isso não há evidência suficiente na solicitação 3054 para afirmar que a campanha foi finalmente retomada. As consultas atuais aos endpoints produtivos do experimento 88 e de retomadas responderam HTTP 503, logo o estado comercial final permanece não confirmado nesta análise e não deve ser inferido dos testes ou merges.
- Fontes consultadas: `GET https://iahub.xyz/api/codex/requests/3054`, API pública do GitHub para PRs, arquivos e workflows dos merges 5310/5311, e tentativas de leitura de `GET http://191.252.181.168:8000/api/experiments/88` e `/api/facebook-campaign-resumptions/experiments/88`. Nenhum token, segredo ou raciocínio interno foi registrado; foram usados apenas o resumo público da execução, sua resposta final técnica e metadados públicos.

### Correção — espera por capacidade do modelo

- Solicitação: evitar que uma indisponibilidade temporária de capacidade encerre novas tarefas como ocorreu na 3054. Pergunta explícita: **“por que esse erro aconteceu?”** O orquestrador já retomava a mesma thread em falhas transitórias de conexão, mas o classificador não reconhecia `Selected model is at capacity`; por isso a mensagem atravessava o limite de retry e convertia imediatamente o job em `FAILED`. O início do `turn/start` também ficava fora do bloco de retry, então uma rejeição de capacidade nessa chamada igualmente escapava sem espera.
- Correção: capacidade passou a ter política própria, sem ampliar retries de erros comuns. Por padrão, a solicitação permanece ativa e tenta a mesma thread até 13 vezes com intervalos de cinco minutos, cobrindo aproximadamente uma hora; os valores são configuráveis por `CODEX_APP_SERVER_CAPACITY_TURN_MAX_ATTEMPTS` e `CODEX_APP_SERVER_CAPACITY_TURN_RETRY_DELAY_MS`. Cada retomada orienta o modelo a conferir os arquivos e o estado existentes antes de repetir ações, reduzindo risco de efeito duplicado. A chamada inicial de `turn/start` e a falha notificada por `turn/completed` usam a mesma proteção.
- Limite deliberado: não há espera infinita. Se a capacidade não voltar dentro da janela configurada, a tarefa falha explicitamente; cancelamento, erro permanente e falhas comuns mantêm seus contratos anteriores. A solicitação histórica 3054 não é reaberta retroativamente.
- Validação: regressão simulou primeiro uma rejeição direta de `turn/start`, depois um `turn/completed` com falta de capacidade e, na terceira tentativa, a recuperação. O job terminou `COMPLETED`, manteve a mesma thread, arquivou-a uma única vez e registrou duas esperas. O teste direcionado da recuperação de conexão e capacidade passou 2/2. Na suíte completa, 189/191 passaram; as duas falhas são limitações preexistentes do ambiente desta sandbox, que não possui o binário `shellcheck`: uma asserção de inventário esperava encontrá-lo e um cenário tentou executá-lo. Build TypeScript e todos os casos relacionados à alteração passaram.

## 2026-09-24 — Comportamento da seleção de produto nas solicitações

- Solicitação: explicar o que ocorre ao escolher um produto na tela de solicitações. Escopo de leitura e documentação; nenhuma regra funcional foi alterada.
- A lista de produtos só aparece no perfil `CHATGPT_CODEX_MKT`. O frontend consulta `GET /products`, mantém apenas itens com `id`, `name` e `slug` válidos, ordena pelo nome e guarda localmente o `slug` selecionado. Trocar a seleção, por si só, não chama execução, não altera o produto e não inicia processo, campanha, agente ou cobrança.
- Ao enviar uma mensagem, se ainda houver um produto selecionado, o frontend procura esse `slug` na lista carregada e antepõe ao prompt a instrução para ler `http://191.252.181.168:8000/api/products/public/{slug}/marketing-definition.md` como fonte de verdade do PDE. Em seguida acrescenta modo, histórico, itens opcionais e a última mensagem. Assim, o efeito real atual da seleção é orientar o modelo a consultar a definição de marketing daquele produto antes de responder ou agir.
- O pedido enviado a `POST /codex/requests` não contém `productId` nem `productSlug`: envia o prompt já composto, a mensagem original, ambiente, modelo, raciocínio, perfil, processo opcional e anexos/itens opcionais. O DTO e a persistência do backend também não possuem vínculo próprio com produto; o backend salva o texto do prompt e repassa esse texto ao sandbox. Consequências: a seleção fica registrada apenas indiretamente na URL incorporada ao prompt; não há integridade referencial produto↔solicitação; escolher produto não seleciona automaticamente o processo correspondente; e mudar o seletor depois do envio não altera solicitações já criadas.
- Sem produto selecionado, nenhuma instrução de fonte de produto é incluída. A conversa e o ambiente continuam compondo o contexto normalmente. Portanto, para tarefas específicas de um produto, selecionar o produto reduz ambiguidade para o modelo, mas atualmente funciona como contexto textual, não como associação estruturada ou autorização operacional.
- Fontes consultadas: `apps/frontend/src/pages/CodexChatgptPage.tsx`, `apps/backend/src/main/java/com/aihub/hub/dto/CreateCodexRequest.java` e `apps/backend/src/main/java/com/aihub/hub/service/CodexRequestService.java`.

### Ajuste — remoção da leitura obrigatória da definição pública

- Solicitação: retirar do prompt a instrução “Antes de começar leia o documento em .../marketing-definition.md e use como fonte de verdade sobre o PDE”. Pergunta explícita: **“por que essa instrução era incluída?”** Como a solicitação não possui associação estruturada com produto, a implementação anterior transformava a seleção em uma URL imperativa dentro do prompt para transmitir contexto ao modelo. Isso acoplava toda solicitação MKT ao endereço interno e tratava um documento remoto como fonte obrigatória, mesmo quando a mensagem não exigia essa leitura.
- Correção: removida a URL e a ordem de leitura. Para o seletor não se tornar um controle sem efeito, o prompt recebe somente o contexto neutro `Produto selecionado pelo usuário: <nome> (slug: <slug>).`; o modelo não é instruído a abrir documento algum nem a tratá-lo como fonte de verdade. Histórico, modo, itens opcionais, processo e mensagem permanecem inalterados.
- Validação: busca no frontend confirmou ausência de `marketing-definition.md` e da frase removida; lint e build do frontend aprovados. Não houve mudança visual, de backend, persistência, autorização ou execução operacional.

## 2026-09-24 — Produto exclusivamente informativo nas solicitações

- Solicitação confirmada: retirar toda informação de produto dos prompts enviados ao modelo; tornar somente o nome obrigatório no cadastro; e exibir o nome selecionado no histórico como já ocorre com processo.
- Pergunta explícita: **“por que o produto ainda chegava ao modelo e exigia slug/ID externo?”** A primeira implementação reutilizava `slug` para compor uma instrução textual no prompt e tratava `externalId` como requisito de integração. Mesmo após remover a URL, o nome e o slug continuavam no prompt porque o seletor ainda era usado na composição. Além disso, a solicitação não possuía campo próprio de produto, portanto não conseguia exibi-lo no histórico sem contaminar o texto destinado ao modelo.
- Correção de contrato: produto mantém apenas `name` como dado de negócio editável. `slug` e `external_id` foram removidos dos DTOs, entidade, serviço, repositório, formulário, tabela e schema; o `id` técnico autogerado continua interno apenas para editar/excluir registros e não é solicitado ao usuário. A migration V55 cobre MySQL, PostgreSQL e H2.
- Separação de responsabilidades: o frontend não acrescenta nome, slug, URL ou qualquer outra informação do produto ao prompt. No envio, `productName` segue como metadado irmão de `processId`; o backend normaliza e persiste esse snapshot informativo em `codex_requests.product_name`, mas despacha ao sandbox somente o prompt original já composto sem produto.
- Histórico: quando selecionado, o nome aparece como `Produto: <nome>` na lista de solicitações e como campo `Produto` no detalhe. Solicitações sem seleção continuam sem produto no histórico. Alterar ou excluir posteriormente o cadastro não reescreve o snapshot histórico.
- Validação: backend completo e migrations H2 aprovados; o teste de serviço comprova simultaneamente que `productName` é persistido e que o prompt permanece exatamente igual. Lint e build do frontend aprovados; busca estática confirmou ausência de produto na função que compõe o prompt. A alteração visual limita o cadastro a um campo Nome e acrescenta o rótulo informativo ao histórico/detalhe.

## 2026-09-24 — Atualização do GitHub CLI solicitada pela execução 3060

- Solicitação: executar a melhoria de ambiente sugerida pela solicitação 3060: atualizar o GitHub CLI para evitar a falha de edição de PR relacionada ao Projects Classic. A entrega funcional da 3060 já estava concluída no Marketing Hub pelo PR 5323; este trabalho altera somente a imagem versionada da sandbox do AI Hub.
- Pergunta explícita: **“por que esse erro aconteceu?”** A imagem instalava `gh` diretamente pelo `apt` do Debian Bookworm, sem versão fixada. Assim, o binário efetivo dependia do snapshot do repositório da distribuição e podia permanecer numa geração que ainda consultava campos GraphQL removidos de Projects Classic durante `gh pr edit`. A atualização feita pela execução 3060 via API oficial contornou aquela ocorrência, mas não corrigia a origem para execuções futuras porque o Dockerfile continuava reproduzindo a versão antiga.
- Alternativas consideradas: (1) manter `apt` e usar `gh api` como fallback, baixo esforço mas preserva a falha do comando normal; (2) adicionar o repositório apt do GitHub, atualização automática porém menos reprodutível; (3) fixar o release oficial, validar checksum e atualizar deliberadamente, maior controle e mesmo padrão já usado para `actionlint`. Escolhida a opção 3.
- Implementação: removido o pacote `gh` da instalação Debian e adicionado `GH_CLI_VERSION=2.101.0`, release mais recente retornado em 24/09/2026 por `GET https://api.github.com/repos/cli/cli/releases/latest`. O build escolhe `amd64` ou `arm64`, baixa tarball e arquivo de checksums do release oficial `cli/cli`, valida SHA-256 antes de extrair e confirma com `gh --version`. Versão, origem oficial, checksum e ausência de regressão para `apt-get install gh` estão protegidos por teste.
- Validação: download real do artefato Linux amd64 e checksum oficial aprovados; o binário retornou `gh version 2.101.0 (2026-09-15)`. Build TypeScript e teste direcionado do contrato do Dockerfile aprovados. Nenhuma credencial GitHub foi necessária ou exposta e nenhuma edição adicional foi feita no PR histórico 5323.

## 2026-09-24 — Correção da migration MySQL V55

- Solicitação: corrigir o deploy no GitHub que não conseguia iniciar o backend e encerrava a verificação interna após 30 tentativas. O `wget` recusado era consequência de o processo Java ter terminado durante a inicialização, e não uma falha de rede ou do frontend.
- Pergunta explícita: **“por que esse erro aconteceu?”** A migration V55 pressupunha que os índices únicos antigos ainda se chamavam `uk_products_slug` e `uk_products_external_id`. O banco usado no deploy não possuía ao menos o primeiro nome, então o MySQL 5.7 abortou no `DROP INDEX` antes de remover as colunas. A migration acoplou indevidamente a remoção das colunas ao nome físico dos índices, embora o MySQL remova automaticamente os índices que dependem de uma coluna quando ela é excluída.
- Correção na causa: a variante MySQL agora remove diretamente `slug` e `external_id`, sem tentar remover índices por nome, e depois adiciona `product_name` como antes. Isso aceita tanto o schema criado pela V38 quanto bancos equivalentes cujos índices tenham nomes diferentes, sem mascarar a ausência das próprias colunas.
- Cobertura: adicionado teste que reproduz um schema MySQL compatível com índices únicos sem os nomes legados, executa a V55 e confirma a remoção das duas colunas e a criação de `codex_requests.product_name` com tamanho 150.

### Complemento — conexão encerrada durante o rebuild da tabela

- Solicitação: corrigir a nova falha do deploy em que a V55 permaneceu sem registro no histórico e o Flyway informou `Connection is closed` ao restaurar o estado e liberar o named lock.
- Pergunta explícita: **“por que esse erro aconteceu?”** A remoção das colunas reconstrói `products` no MySQL 5.7 e ultrapassou os 30 segundos configurados globalmente em `socketTimeout`. O Connector/J encerrou a conexão enquanto o servidor ainda executava DDL; como DDL no MySQL possui commit implícito, a alteração podia terminar no servidor sem que o Flyway conseguisse registrar a V55. O retry então encontraria um schema parcialmente alterado. A mensagem de named lock e a falha de restauração eram consequências dessa conexão já fechada, não a causa.
- Correção na causa: o timeout de abertura continua em cinco segundos e consultas JPA continuam com limite de 30 segundos, mas o timeout indiscriminado de leitura do socket passa a ser desabilitado por padrão (`0`), permitindo que migrations longas concluam na mesma conexão. `DB_SOCKET_TIMEOUT_MS` continua disponível para override consciente por ambiente.
- Recuperação segura: a V55 MySQL passou a consultar `information_schema` e montar apenas os `ALTER TABLE` ainda necessários. As duas colunas de `products` são removidas juntas quando ambas existem, evitando dois rebuilds; se uma execução anterior já confirmou parte do DDL, a próxima tentativa retoma somente o que falta. A criação de `product_name` também é condicional. O teste protege a independência dos nomes físicos de índices e as ramificações necessárias para retomada após commit implícito.

## 2026-09-24 — Modelo e raciocínio associados ao produto

- Solicitação: permitir associar modelo e tipo de raciocínio a cada produto, aplicar automaticamente essas preferências ao selecionar o produto na solicitação MKT e limpar produto/processo depois de cada envio aceito.
- Pergunta explícita de causa raiz: **“por que esse comportamento não existia?”** O produto havia sido deliberadamente reduzido a um metadado informativo composto apenas pelo nome; seu contrato, entidade e tabela não possuíam campos de preferência de execução. Por isso o seletor MKT não tinha dados dos quais derivar modelo/raciocínio e o fluxo de sucesso limpava apenas prompt, anexos e itens auxiliares, preservando produto e processo por omissão.
- Correção na origem: o contrato e a persistência de produto agora possuem associações opcionais `modelName` e `reasoningEffort`, com migrations V56 para H2, MySQL e PostgreSQL e validação dos níveis aceitos. O cadastro carrega os modelos cadastrados, permite definir/remover as duas associações e as exibe na listagem.
- Integração MKT: `GET /products` entrega as preferências estruturadas. Quando um produto é escolhido, a tela troca o modelo (se ele estiver disponível na conta/lista atual) e o nível de raciocínio conforme o cadastro; associações ausentes não sobrescrevem as escolhas manuais. Depois de `POST /codex/requests` ser aceito, produto e processo voltam respectivamente para “Sem produto selecionado” e “Sem processo selecionado”. Em caso de erro no envio, as seleções são preservadas para permitir correção e nova tentativa.
- Validação: build TypeScript/Vite do frontend aprovado; suíte Maven do backend executada para validar compilação, migrations e regressões. A alteração visual abrange o formulário/listagem de produtos e o comportamento dos seletores da tela MKT.

### Complemento — produto e processo no cartão do diálogo
